...
int main(int argc, char **argv) {
    ...
    for (const auto &entry : pallet::catalog->maps) {
        ...
        const auto &houses = pallet::houses(scene);
        for (auto h : houses) {
            if (h.w <= 0 || h.d <= 0 || h.x < 0 || h.z < 0 || h.x + h.w > scene.width ||
                h.z + h.d > scene.height) {
                std::fprintf(stderr, "FAIL: invalid building in map %d\n", scene.id);
                return 4;
            }
            std::printf("building,%d,%.1f,%.1f,%.1f,%.1f,%.1f\n", scene.id, h.x, h.z, h.w, h.d,
                        h.eave);
            ++total;
        }
        ...
        for (int z = 0; z < scene.height; z++)
            for (int x = 0; x < scene.width; x++) {
                ...
                int top = pallet::map_tile(rom.data(), scene, x * 2, z * 2),
                    bottom = pallet::map_tile(rom.data(), scene, x * 2, z * 2 + 1);
        ...
        for (auto w : scene.warps) {
            ...
            if ((scene.tileset != 0 && scene.tileset != 23) || tile != 0x1b)
                continue;
            bool covered = pallet::terrain(rom.data(), scene, w.x, w.z) == pallet::Terrain::Portal;
            for (auto h : houses)
                covered |= w.x >= h.x && w.x < h.x + h.w && w.z == h.z + h.d - 1;
            if (!covered) {
                std::fprintf(stderr, "MISSING building: map=%d door=%d,%d\n", scene.id, w.x, w.z);
                ++missing;
            }
        }
    }
