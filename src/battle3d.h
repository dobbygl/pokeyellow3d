...
namespace battle3d {
GLuint texture=0;
struct Portrait {battle::Image image{};int species=0;float alpha=0,hp=0,damage=0;int last_hp=0;uint64_t fingerprint=0;};
...
void shutdown() {if(texture)glDeleteTextures(1,&texture);texture=0;terrain_known=false;reset();}
bool initialize_texture() {
    if(texture)return true;
    std::vector<uint8_t> data(AW*AH*4);
    for(int i=0;i<4;i++)data[((AH-1)*AW+AW-1)*4+i]=255;
...
firstperson::Matrix camera(float aspect,double time) {
    ...
    return {f/aspect,0,0,0, 0,f*c,a*s,-s, 0,-f*s,a*c,-c,
        -f*ex/aspect,f*(-c*ey+s*ez),-a*(s*ey+c*ez)+b,s*ey+c*ez};
}
...
void orb(std::vector<Vertex>& v,Vec center,float radius,Color color,bool ball=false) {
    for(int row=0;row<8;row++)for(int col=0;col<16;col++) {
        auto point=[&](int r,int c) {
            ...
            return Vec{center.x+radius*std::cos(a)*std::cos(b),center.y+radius*std::sin(a),center.z+radius*std::cos(a)*std::sin(b)};
        };
        Color shade_color=ball?(row<4?Color{.94f,.94f,.89f}:Color{.86f,.19f,.13f}):color;
        if(ball&&(row==3||row==4))shade_color={.13f,.17f,.19f};
        quad(v,point(row,col),point(row,col+1),point(row+1,col+1),point(row+1,col),shade(shade_color,.7f+.3f*row/7));
    }
}
Color move_color(int type) {
    switch(type) {
    case 20:return {.99f,.35f,.12f};case 21:return {.25f,.65f,1};
    case 22:return {.42f,.86f,.3f};case 23:return {1,.89f,.18f};
    case 24:return {.95f,.4f,.86f};case 25:return {.57f,.94f,1};
    default:return {.81f,.72f,1};
    }
}
void effects(std::vector<Vertex>& v,const GBContext* ctx) {
    if(capture_effect) {
        int id=battle::read(ctx,battle::Animation),counter=battle::read(ctx,battle::AnimCounter);
        ...
        Vec p=toss?Vec{-2+4*t,1.1f+3*std::sin(t*firstperson::Pi),2-5*t}:Vec{2+wobble,.46f,-3};
        orb(v,p,.3f,White,true);
        if(id==0xc3)for(int i=0;i<12;i++) {
            float a=2*firstperson::Pi*i/12,r=.4f+.9f*std::fmod(effect_time,1.f);
            orb(v,{2+std::cos(a)*r,1.4f+std::sin(a)*r,-3},.10f,{.96f,.93f,.72f});
        }
        return;
    }
    if(effect_kind==battle::Effect::Original||effect_kind==battle::Effect::Physical)return;
    auto move=battle::move(ctx,effect_id);Color color=move_color(move.type);
    ...
    if(effect_kind==battle::Effect::Projectile) {
        ...
        for(int i=6;i>=0;i--) {
            ...
            orb(v,{sx+(tx-sx)*p,1.6f+.5f*std::sin(p*firstperson::Pi),sz+(tz-sz)*p},.20f-i*.02f,shade(color,1-i*.08f));
        }
    } else {
        for(int i=0;i<16;i++) {
            float a=i*2*firstperson::Pi/16+effect_time*3,r=.8f+.3f*std::sin(effect_time*9);
            float height=effect_kind==battle::Effect::Self?.3f+2.8f*std::fmod(t+i/16.f,1.f):1.6f+std::sin(a)*1.2f;
            orb(v,{tx+std::cos(a)*r,height,tz+.2f*std::sin(a)},.085f,color);
        }
    }
}
void panel(const GBContext* ctx,bool enemy,float w,float h) {
    auto m=battle::mon(ctx,enemy);auto* dl=ImGui::GetForegroundDrawList();
    ...
    char label[80];std::snprintf(label,sizeof label,"%s   Lv.%d",m.name.c_str(),m.level);
    dl->AddText(nullptr,18*scale,{x+14*scale,y+12*scale},IM_COL32(248,244,219,255),label);
    int index=enemy?1:0;float hp=std::clamp(portraits[index].hp/std::max(1,m.max_hp),0.f,1.f);
    ImU32 color=m.hp_color==2?IM_COL32(231,92,70,255):m.hp_color==1?IM_COL32(237,192,71,255):IM_COL32(110,194,139,255);
    dl->AddRectFilled({x+14*scale,y+42*scale},{x+pw-14*scale,y+50*scale},IM_COL32(44,59,58,255),3*scale);
    dl->AddRectFilled({x+14*scale,y+42*scale},{x+14*scale+(pw-28*scale)*hp,y+50*scale},color,3*scale);
    std::snprintf(label,sizeof label,"%d / %d   %s",m.hp,m.max_hp,battle::status(m.status));
    dl->AddText(nullptr,13*scale,{x+14*scale,y+58*scale},IM_COL32(182,201,190,255),label);
    if(!enemy) {
        float xp=battle::experience(ctx);
        dl->AddRectFilled({x+14*scale,y+83*scale},{x+pw-14*scale,y+87*scale},IM_COL32(44,59,58,255));
        dl->AddRectFilled({x+14*scale,y+83*scale},{x+14*scale+(pw-28*scale)*xp,y+87*scale},IM_COL32(112,172,230,255));
    }
...
void draw(GBContext* ctx,int w,int h,bool menu_open) {
    ...
    float dt=last_cycles&&ctx->cycles>last_cycles?std::min(.1f,float(ctx->cycles-last_cycles)/4194304):1.f/60;
    if(!shown||ctx->cycles<last_cycles) {
        portraits={};overlay_alpha=effect_time=0;effect_id=effect_actor=0;
    ...
    bool tail=!animation&&cached&&effect_kind==battle::Effect::Self&&effect_actor==actor&&
        ctx->cycles>=effect_started&&ctx->cycles-effect_started<uint64_t(.55*4194304);
    int id=animation?battle::read(ctx,battle::Animation):tail?effect_id:0;
    if(id!=effect_id||actor!=effect_actor){effect_started=ctx->cycles;effect_time=0;}
    else effect_time=float(ctx->cycles-effect_started)/4194304;
    effect_id=id;effect_actor=actor;
    ...
    for(int i=0;i<2;i++) {
        ...
        if(intro) {
            ...
            if(image!=p.image) {
                p.image=image;p.fingerprint=battle::fingerprint(image);
                glTexSubImage2D(GL_TEXTURE_2D,0,64,0,56,56,GL_RGBA,GL_UNSIGNED_BYTE,image.data());
            }
            p.alpha=std::min(1.f,p.alpha+dt*7);continue;
        }
        if(p.species!=m.species){p={};p.species=m.species;p.hp=float(m.hp);p.last_hp=m.hp;}
        else p.hp+=(m.hp-p.hp)*(1-std::exp(-dt*12));
        if(m.hp<p.last_hp)p.damage=.4f;else p.damage=std::max(0.f,p.damage-dt);
        p.last_hp=m.hp;
        ...
        if(present&&!animation) {
            ...
            if(image!=p.image) {
                p.image=image;glTexSubImage2D(GL_TEXTURE_2D,0,i*64,0,56,56,GL_RGBA,GL_UNSIGNED_BYTE,p.image.data());
    ...
    const Color grounds[]={{.46f,.43f,.31f},{.35f,.47f,.32f},{.23f,.43f,.49f},{.27f,.31f,.32f},{.43f,.42f,.49f}};
    Color ground=grounds[terrain];std::vector<Vertex> v;
    ...
    for(int z=-15;z<12;z++)for(int x=-12;x<12;x++)if((x+z)&1)
        quad(v,{float(x),0,float(z)},{float(x+1),0,float(z)},
            {float(x+1),0,float(z+1)},{float(x),0,float(z+1)},shade(ground,.97f));
    if(intro)platform(v,0,-2,shade(ground,1.4f));
    else {platform(v,2,-3,shade(ground,1.4f));platform(v,-2,2,shade(ground,1.4f));}
    for(int i=0;i<2;i++) {
        float x=i?2:-2,z=i?-3:2,size=i?2.8f:3.1f,base=.14f;
        if(intro){if(!i)continue;x=0;z=-2;size=4;}
        if(effect_kind==battle::Effect::Physical&&i==effect_actor) {
            float lunge=std::max(0.f,std::sin(effect_time*7));x+=(i?-1:1)*.9f*lunge;z+=(i?1:-1)*1.1f*lunge;
        }
        auto& portrait=portraits[i];
        if(portrait.damage>0)x+=std::sin(portrait.damage*90)*.14f;
        float alpha=overlay_alpha>0?0:portrait.alpha;
        if(portrait.damage>0&&int(portrait.damage*25)%2)alpha*=.3f;
        if(capture_effect&&i==1&&(id==0xc2||id==0xc8||!battle::rectangle(ctx,battle::Enemy)))alpha=0;
        quad(v,{x-size/2,base+size,z},{x+size/2,base+size,z},
            {x+size/2,base,z},{x-size/2,base,z},{1,1,1,alpha},{float(i*64),0,56,56});
    }
    if(overlay_alpha==0)effects(v,ctx);
    glBindBuffer(GL_ARRAY_BUFFER,buffer);glBufferData(GL_ARRAY_BUFFER,v.size()*sizeof(Vertex),v.data(),GL_STREAM_DRAW);
