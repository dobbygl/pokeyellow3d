...
inline bool far_call(const GBContext *ctx, size_t call, int target, int bank) {
    ...
    return supported(ctx) && call >= 5 && call + 2 < ctx->rom_size && ctx->rom[call - 5] == 0x21 &&
           ctx->rom[call - 4] == (target & 255) && ctx->rom[call - 3] == (target >> 8) &&
           ctx->rom[call - 2] == 0x06 && ctx->rom[call - 1] == bank &&
...
inline bool visible_hud(const GBContext *ctx, const uint32_t *lcd) {
    if (!lcd || !introduction(ctx) || !(ctx->io[0x40] & 0x80) || ctx->io[0x47] != 0xe4)
        return false;
...
inline Sample sample(const GBContext *ctx, const uint32_t *lcd = nullptr) {
    ...
    if (entry(ctx)) {
        ...
        if (transition && battle::live_return(ctx, 0x70d75, 0x4bd0))
            return {Phase::Flash};
        if (ctx->io[0x47] == 255)
        ...
        constexpr std::array<std::array<int, 2>, 8> calls{{{0x70aeb, 0x4b1d},
                                                           {0x70b0c, 0x1e64},
                                                           {0x70c31, 0x372f},
                                                           {0x70c76, 0x4d8f},
                                                           {0x70d0a, 0x4d8f},
        ...
        if (transition)
            for (auto call : calls)
                if (battle::live_return(ctx, call[0], call[1])) {
                    int filled = 0;
                    for (int i = 0; i < 360; i++)
                        filled += battle::read(ctx, battle::TileMap + i) == 255;
                    return {Phase::Wipe, float(filled) / 360.f};
                }
        return {Phase::Preparing};
    }
    if (introduction(ctx))
        return {Phase::Introduction, 0, visible_hud(ctx, lcd)};
    if (loop(ctx))
        return {Phase::Battle};
    if (ending(ctx))
        return {Phase::Exit};
