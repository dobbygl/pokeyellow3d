...
inline bool valid_live_map(const GBContext *ctx, const Scene &s) {
    ...
    if ((bw + 6) * (bh + 6) > 1300)
        return false; // wOverworldMap's canonical allocation.
    const auto &valid = tileset(s).valid_blocks;
    for (int z = 0; z < bh; z++)
        for (int x = 0; x < bw; x++)
            if (!valid[read(ctx, uint16_t(0xc6e8 + (z + 3) * (bw + 6) + x + 3))])
                return false;
    return true;
}

inline View view(const GBContext *ctx) {
    if (!ctx || !ctx->wram || !ctx->rom || !ctx->io || !ctx->vram || ctx->rom_size != 1048576)
        return View::Unsupported;
    if (!load_catalog(ctx->rom, ctx->rom_size))
        return View::Unsupported;
    const Scene *s = ensure_scene(read(ctx, Map));
    if (read(ctx, Battle))
        return s && battle::ready(ctx) ? View::Battle : View::Unsupported;
    if (dex_area_state::active(ctx))
        return View::PokedexArea;
    if (dex_state::data(ctx) || dex_state::list(ctx).number)
        return View::Pokedex;
    if (pc_state::sample(ctx).mode != pc_state::Mode::None)
        return View::Computer;
    if (!s || read(ctx, Width) * 2 != s->width || read(ctx, Height) * 2 != s->height ||
        read(ctx, Tileset) != s->tileset || read(ctx, Battle) != 0 || read(ctx, X) >= s->width ||
        read(ctx, Y) >= s->height)
        return View::Unsupported;
    if (!(ctx->io[0x40] & 0x80) || (ctx->io[0x47] != 0xe4) || read(ctx, UpdateSprites) == 0 ||
        !read(ctx, Sprite1))
...
inline bool bottom_dialogue(const GBContext *ctx) {
    if (!ctx || !ctx->wram || !ctx->io || read(ctx, Battle) || ctx->io[0x47] != 0xe4)
        return false;
    ...
    for (int y = 0; y < 12; y++)
        for (int x = 0; x < 20; x++)
            if (tile(x, y) >= 0x60)
                return false;
    if (tile(0, 12) != 0x79 || tile(19, 12) != 0x7b || tile(0, 17) != 0x7d || tile(19, 17) != 0x7e)
        return false;
    for (int x = 1; x < 19; x++)
        if (tile(x, 12) != 0x7a || tile(x, 17) != 0x7a)
            return false;
    for (int y = 13; y < 17; y++)
        if (tile(0, y) != 0x7c || tile(19, y) != 0x7c)
            return false;
    return true;
}
...
inline std::array<float, 2> world_player(const GBContext *ctx) {
    auto p = player(ctx);
    if (const auto *s = scene(read(ctx, Map))) {
        p[0] += s->origin_x;
        p[1] += s->origin_z;
    }
    return p;
