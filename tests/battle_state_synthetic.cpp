...
int main() {
    try {
        ...
        check(!battle::normal(ctx) && !battle::ready(ctx), "the old man tutorial is never composed");
        ...
        check(battle::rectangle(ctx, battle::Player, true), "the player rectangle is also on screen");
        ...
        check(!battle::rectangle(ctx, battle::Enemy, true), "a BG map that does not show the picture");
        machine.vram[0x1800 + 3 * 32 + 14] = saved_map;
        machine.tile(14, 3, 0);
        check(!battle::rectangle(ctx, battle::Enemy), "a hole in the tile map breaks the rectangle");
        ...
        for (const char* name : {"PI", "PIKA", "PIKACH"}) {
            ...
            check(battle::name_matches(ctx, 0xcfd9, 1, 0), "the enemy name sits where CenterMonName put it");
            check(battle::name_matches(ctx, 0xd008, 10, 7), "the player name sits where CenterMonName put it");
        ...
        const std::array<std::array<uint8_t, 3>, 4> monochrome{{{255, 255, 255}, {170, 170, 170}, {85, 85, 85}, {0, 0, 0}}};
        check(battle::palette(ctx, enemy_species) == monochrome, "species without a dex number keep the default ramp");
        ...
        auto alpha = [](const battle::Image& image, int x, int y) { return image[size_t(y * 56 + x) * 4 + 3]; };
        ...
        check(!alpha(front, 0, 0) && !alpha(front, 55, 0) && !alpha(front, 2, 30) && !alpha(front, 53, 30),
              "the background around the body is transparent");
        check(!alpha(back, 0, 0) && !alpha(back, 55, 55), "the back picture clears its background too");
        check(alpha(front, 8, 20) == 255 && alpha(back, 8, 20) == 255, "the outline is never cleared");
        check(alpha(front, 20, 20) == 255 && alpha(back, 20, 20) == 255,
              "white enclosed by the outline stays opaque");
        check(alpha(front, 35, 32) == 255 && alpha(front, 20, 45) == 255, "shaded patches stay opaque");
        ...
        check(back_clear == 56 * 56 - 40 * 48, "the back picture clears the background and nothing else");

        // --- mon() and status() -------------------------------------------------
        auto enemy = battle::mon(ctx, true);
        auto player = battle::mon(ctx, false);
        check(enemy.species == enemy_species && enemy.hp == 18 && enemy.max_hp == 24, "enemy species and HP words");
        check(enemy.level == 7 && enemy.hp_color == 1 && enemy.name == "PIKACH", "enemy level, bar colour and name");
        check(player.species == player_species && player.hp == 26 && player.max_hp == 31, "player species and HP words");
        check(player.level == 9 && player.hp_color == 1 && player.name == "BULBAS", "player level, bar colour and name");
        check(std::string(battle::status(enemy.status)).empty(), "a healthy mon has no status");
        for (auto probe : {std::pair<int, const char*>{1, "SLP"}, {7, "SLP"}, {8, "PSN"},
                           {16, "BRN"}, {32, "FRZ"}, {64, "PAR"}}) {
            machine.write(0xcfe8, probe.first);
            check(std::string(battle::status(battle::mon(ctx, true).status)) == probe.second, "status name");
        }
        ...
        check(!battle::animation_running(ctx), "an animation id alone proves nothing: the ids are unions");
        machine.arm_live_return(0x3f09b, 0x3eb4);
        check(battle::animation_running(ctx), "the verified CALL plus its return address on the stack");
        ...
        check(battle::trainer_intro(ctx) && battle::ready(ctx), "the trainer introduction is composed");
