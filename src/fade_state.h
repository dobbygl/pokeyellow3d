...
struct Tone {float multiply=1,add=0;};
constexpr Tone tone(uint8_t bgp) {
    int sum=0;for(int i=0;i<4;i++)sum+=3-((bgp>>(i*2))&3);
    return sum<=6?Tone{sum/6.f,0}:Tone{(12-sum)/6.f,(sum-6)/6.f};
}
constexpr bool palette(uint8_t bgp) {
    return bgp==0xe4||bgp==0xf9||bgp==0xfe||bgp==0xff||
        bgp==0x90||bgp==0x40||bgp==0;
}
...
inline bool home_call(const GBContext* ctx,size_t call,uint16_t target,uint8_t opcode=0xcd) {
    if(!ctx||!ctx->rom||!ctx->wram||call+2>=ctx->rom_size||call>=0x4000||
       ctx->rom[call]!=opcode||ctx->rom[call+1]!=(target&255)||ctx->rom[call+2]!=(target>>8)||
       ctx->sp<0xd000||ctx->sp>=0xe000)return false;
    const auto ret=call+3;
    for(int a=ctx->sp;a<0xdfff;a+=2)
        if(ctx->wram[a-0xc000]==(ret&255)&&ctx->wram[a-0xbfff]==(ret>>8))return true;
    return false;
}
inline bool warp(const GBContext* ctx) {
    return home_call(ctx,0x576,0x1eb6)|| // Rock Tunnel's special fade
        ...
        (home_call(ctx,0x1fd,0x750,0xc4)&&
         !home_call(ctx,0x2de,0x2817)&&!home_call(ctx,0x3f47,0x2817));
}
} // namespace fade
