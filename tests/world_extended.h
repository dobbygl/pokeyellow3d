...
struct QaWalk {
    ...
    void tick() {
        ...
        require(pallet3d_active()==(blend.active||(render_enabled&&(state==pallet::View::Overworld || state==pallet::View::Battle || state==pallet::View::Pokedex || pallet3d_area().active || pallet3d_pc().active || pallet3d_battle_transition().active || pallet3d_warp_overlay() || pallet3d_menu().active || (state==pallet::View::Dialogue&&pallet::bottom_dialogue(ctx))))),"scene selection");
