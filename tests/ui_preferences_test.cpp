...
int main() {
    ...
    check(load(path).style == Style::Integrated, "new installations default to integrated");
    for (const char *mode : {"disabled", "automatic", "fixed"}) {
        ...
        check(migrated.lighting.mode == old.mode && migrated.lighting.hour == old.hour &&
                  migrated.style == Style::Integrated,
              "v1 lighting is unchanged; the new style defaults to integrated");
        for (auto style : {Style::Classic, Style::Integrated}) {
            check(save(path, {old, style}), "write v2 preferences");
            auto restored = load(path);
            check(restored.style == style && restored.lighting.mode == old.mode &&
                      restored.lighting.hour == old.hour,
                  "v2 restart restores both independent preferences exactly");
        }
    }
    for (const char *bad :
        ...
        check(fallback.style == Style::Integrated &&
    ...
    std::puts("PASS: v1 migration, v2 persistence, default and invalid preferences");
}
