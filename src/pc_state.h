...
inline Terminal terminal(const GBContext *ctx) {
    if (!ctx || !ctx->wram || !ctx->rom || ctx->rom_size != 1048576 ||
        battle::read(ctx, 0xc109) != 4)
        return {};
    auto *scene = pallet::scene(battle::read(ctx, 0xd35d));
    if (!scene || !scene->interior)
        return {};
    ...
    if ((!bedroom && !center) || cell.kind != interior::Kind::Furniture)
        return {};
...
inline bool far_call(const GBContext *ctx, size_t call, int bank, int address) {
    ...
    return call >= 5 && call + 2 < ctx->rom_size && ctx->rom[call - 5] == 6 &&
           ctx->rom[call - 4] == bank && ctx->rom[call - 3] == 0x21 &&
           ctx->rom[call - 2] == (address & 255) && ctx->rom[call - 1] == (address >> 8) &&
           battle::live_return(ctx, call, 0x3e84);
}
inline Sample sample(const GBContext *ctx) {
    if (!ctx || !ctx->wram || !ctx->rom || !ctx->io || ctx->rom_size != 1048576 ||
    ...
    if (!fade::home_call(ctx, 0x3408, 0x3e84))
        return {};
    auto pc = terminal(ctx);
    if (pc.map < 0)
        return {};
    Mode mode = pc.bedroom ? Mode::Items : Mode::Center;
    if (far_call(ctx, 0x17d3f, 1, 0x778e))
        mode = Mode::Items;
    else if (far_call(ctx, 0x17d51, 7, 0x62ae))
        mode = Mode::Oak;
    else if (far_call(ctx, 0x17d63, 0x1d, 0x5dfe))
        mode = Mode::Hall;
    else if (far_call(ctx, 0x17d87, 8, 0x546f))
        mode = Mode::Bill;
    bool items_menu = mode == Mode::Items && battle::read(ctx, 0xcc24) == 2 &&
                      battle::read(ctx, 0xcc25) == 1 && battle::read(ctx, 0xcc28) == 3 &&
                      battle::tile(ctx, 0, 0) == 0x79 && battle::tile(ctx, 15, 9) == 0x7e;
...
struct Motion {
    ...
    void update(bool wanted, uint32_t now, bool frozen) {
        uint32_t elapsed = initialized ? now - last : 0;
        last = now;
        initialized = true;
        if (frozen) {
            paused = true;
