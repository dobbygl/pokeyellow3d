...
void upload(const uint32_t* original) {
    if(!texture) {
        ...
        for(int i=0;i<4;i++)pixels[((AH-1)*AW+AW-1)*4+i]=255;
    ...
    if(!original|| (lcd_valid&&!std::memcmp(lcd.data(),original,sizeof(lcd))))return;
    std::copy_n(original,lcd.size(),lcd.begin());lcd_valid=true;
    std::array<uint8_t,160*144*4> pixels{};
    for(size_t i=0;i<lcd.size();i++){pixels[i*4]=lcd[i]>>16;pixels[i*4+1]=lcd[i]>>8;pixels[i*4+2]=lcd[i];pixels[i*4+3]=255;}
...
void draw(GBContext* ctx,int w,int h,bool menu_open) {
    ...
    close.x=center.x;close.y=center.y;close.z=center.z+std::max(.87f,.48f*1.428148f/(float(w)/h*.76f));close.yaw=0;
    auto target=close.perspective(float(w)/h);matrix=world_frame.matrix;
    auto normalize=[&](firstperson::Matrix& m) {
        float divisor=m[3]*center.x+m[7]*center.y+m[11]*center.z+m[15];
        if(divisor>.0001f)for(auto& value:m)value/=divisor;
    };
    normalize(matrix);normalize(target);
    for(size_t i=0;i<16;i++)matrix[i]=matrix[i]*(1-t)+target[i]*t;
    ...
    if(main) {upload(gb_get_framebuffer(ctx));monitor_image=true;}
    ...
    for(int i=0;i<4;i++) {
        ...
        screen[i*2+1]=(1-(matrix[1]*p.x+matrix[5]*p.y+matrix[9]*p.z+matrix[13])/d)*h/2;
    }
    glUseProgram(program);glUniformMatrix4fv(matrix_loc,1,GL_FALSE,matrix.data());
    glUniform2f(fade_loc,1,0);glUniform1f(fog_loc,0);glUniform1f(sky_loc,0);glUniform1f(xray_loc,0);
    ...
    if(open&&!main) {
        if(full) {
            scene_filter::capture(w,h);menu_blurred=scene_filter::draw(w,h);
            if(!menu_open)lcd_overlay::framed(gb_get_framebuffer(ctx),float(w),float(h));
        } else if(!menu_open)lcd_overlay::regions(gb_get_framebuffer(ctx),menu_regions,float(w),float(h));
    }
    scene_filter::invalidate();presented=true;
}
} // namespace pc3d
