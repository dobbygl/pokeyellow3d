...
struct WarpPoint {
    int x = 0, z = 0, destination = 0, entrance = 0;
};
struct SignPoint {
    int x = 0, z = 0, text = 0;
};
struct ActorPoint {
    int sprite = 0, x = 0, z = 0, movement = 0, range = 0, text = 0;
};
...
constexpr size_t OutdoorBank = 8, OutdoorBlocks = OutdoorBank * 0x4000,
                 OutdoorGraphics = OutdoorBlocks + 0x1000;
constexpr size_t IndoorBank = 9, IndoorBlocks = IndoorBank * 0x4000,
                 IndoorGraphics = IndoorBlocks + 0x1000;
...
struct ConnSpec {
    char direction = 'N';
    int map = 0, y_alignment = 0, x_alignment = 0;
};
struct MapSpec {
...
inline std::vector<uint8_t> build() {
    ...
    auto put_word = [&](size_t at, int v) {
        image[at] = uint8_t(v & 0xff);
        image[at + 1] = uint8_t((v >> 8) & 0xff);
    };
    ...
    auto sheet = [&](size_t at, int salt) {
        for (int tile = 0; tile < 96; tile++)
            for (int row = 0; row < 8; row++) {
                put(at + size_t(tile) * 16 + size_t(row) * 2,
                    uint8_t(tile * 11 + row * 29 + salt * 7));
                put(at + size_t(tile) * 16 + size_t(row) * 2 + 1,
                    uint8_t(tile * 37 + row * 13 + salt * 53 + 0x5a));
            }
    };
    sheet(OutdoorGraphics, 0);
    sheet(IndoorGraphics, 1);
    ...
    auto block_table = [&](size_t at, const std::vector<std::array<uint8_t, 16>> &defined) {
        for (int block = 0; block < 256; block++)
            for (int i = 0; i < 16; i++)
                put(at + size_t(block) * 16 + size_t(i), block < int(defined.size())
                                                             ? defined[size_t(block)][size_t(i)]
                                                             : uint8_t((block * 7 + i * 3) % 96));
    };
    block_table(OutdoorBlocks, {uniform(GroundTop, GroundBottom), uniform(GrassTop, GrassBottom),
                                uniform(TreeTop, TreeBottom), uniform(RockTop, RockBottom),
                                uniform(LedgeTop, LedgeBottom), uniform(WaterTop, WaterBottom),
                                uniform(FenceTop, FenceBottom), uniform(SignTop, SignBottom),
    ...
    for (size_t i = 0; i < 4; i++)
        put(OutdoorCollisions + i,
            std::array<int, 4>{GroundBottom, GrassBottom, LedgeBottom, 0xff}[i]);
    for (size_t i = 0; i < 4; i++)
        put(IndoorCollisions + i, std::array<int, 4>{FloorBottom, 0x0d, 0x1c, 0xff}[i]);

    // --- tileset headers ---------------------------------------------------
    auto tileset_header = [&](int id, size_t bank, size_t blocks, size_t graphics,
                              size_t collisions, int grass) {
    ...
    for (int id = 0; id < kanto::TilesetCount; id++)
        if (id == 1)
            tileset_header(id, IndoorBank, IndoorBlocks, IndoorGraphics, IndoorCollisions, 0xff);
        else
            tileset_header(id, OutdoorBank, OutdoorBlocks, OutdoorGraphics, OutdoorCollisions,
                           GrassBottom);
    ...
    for (auto entry : {std::array<int, 4>{2, GroundBottom, LedgeBottom, 0x80},
        ...
        for (int i = 0; i < 4; i++)
            put(p + size_t(i), entry[size_t(i)]);
        p += 4;
    }
    put(p, 0xff);
    p = LandPairs;
    for (auto entry : {std::array<int, 3>{0, GroundBottom, WaterBottom},
                       std::array<int, 3>{1, FloorBottom, WallBottom}}) {
        for (int i = 0; i < 3; i++)
            put(p + size_t(i), entry[size_t(i)]);
        p += 3;
    }
    put(p, 0xff);
    put(WaterPairs, 0);
    put(WaterPairs + 1, WaterBottom);
    put(WaterPairs + 2, GroundBottom);
    put(WaterPairs + 3, 0xff);
    ...
    auto write_map = [&](const MapSpec &m) {
        ...
        put(header, m.tileset);
        put(header + 1, m.blocks_h);
        put(header + 2, m.blocks_w);
        put_word(header + 3, int(0x4000 + (blocks_at - MapBase)));
        put_word(header + 5, 0);
        put_word(header + 7, 0); // text and script pointers, never read
        int flags = 0;
        for (auto c : m.connections)
            flags |= c.direction == 'N' ? 8 : c.direction == 'S' ? 4 : c.direction == 'W' ? 2 : 1;
        put(header + 9, flags);
        size_t e = header + 10;
        for (char d : {'N', 'S', 'W', 'E'})
            for (auto c : m.connections)
                if (c.direction == d) {
                    put(e, c.map);
                    put_word(e + 1, 0);
                    put_word(e + 3, 0); // strip pointers, never read
                    put(e + 5, m.blocks_w * 2);
                    put(e + 6, m.blocks_w * 2); // strip length and width
                    put(e + 7, c.y_alignment);
                    put(e + 8, c.x_alignment);
                    put_word(e + 9, 0);
                    e += 11;
                }
    ...
    east.blocks_w = 6;
    east.blocks_h = 6;
    east.blocks = std::vector<uint8_t>(36, 0);
    east.blocks[7] = 1; // a grass block at cells (2..3,2..3)
    ...
    for (size_t at = 0; at < image.size(); at++)
        if (at != 0x14e && at != 0x14f)
            global = uint16_t(global + image[at]);
    image[0x14e] = uint8_t(global >> 8);
    image[0x14f] = uint8_t(global & 0xff);
    return image;
}
} // namespace detail
...
inline std::vector<uint8_t> rom() {
    static const std::vector<uint8_t> image = detail::build();
    return image;
}
} // namespace synthetic
