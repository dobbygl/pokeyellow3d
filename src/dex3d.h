...
void shutdown() {
    ...
    texture=0;species=0;picture={};image={};lcd_valid=presented=false;uploads=0;cache={};selection={};list_mode=false;
}
...
void panel(std::vector<Vertex>& v,float left,float top,float w,float h,Color color,UV uv=Solid,float z=.43f) {
    quad(v,{left,top,z},{left+w,top,z},{left+w,top-h,z},{left,top-h,z},color,uv);
}
void lcd_region(std::vector<Vertex>& v,lcd_overlay::Rect r,float left,float top,float scale) {
    panel(v,left,top,r.w*8*scale,r.h*8*scale,White,{float(r.x*8),float(64+r.y*8),float(r.w*8),float(r.h*8)});
}
bool draw(GBContext* ctx,int w,int h,bool menu_open) {
    presented=false;
    auto desired=dex_state::list(ctx);
    bool was_list=list_mode;list_mode=desired.number!=0;
    ...
    if(list_mode) {
        ...
        if(dex_state::visible(ctx,original,desired))selection=desired;
        else if(!was_list||!dex_state::visible(ctx,original,selection))selection={};
        selection.seen=desired.seen;selection.caught=desired.caught;
        selection.registration=dex_state::flag(ctx,dex_state::Owned,selection.number)?dex_state::Registration::Caught:
            dex_state::flag(ctx,dex_state::Seen,selection.number)?dex_state::Registration::Seen:dex_state::Registration::Absent;
    } else selection={};
    if(!list_mode&&!dex_state::data(ctx))return false;
    int current=list_mode?selection.species:battle::read(ctx,dex_state::Current);
    species=current;picture=cache.get(ctx->rom,ctx->rom_size,current,true);
    // Never show a cached previous species or an unverified partial transfer.
    if((!picture.valid&&(!list_mode||selection.number))||(!list_mode&&std::memcmp(picture.tiles.data(),ctx->vram+0x1000,mon_pic::TileBytes)))return false;
    if(!initialize_texture())return false;
    glBindTexture(GL_TEXTURE_2D,texture);
    auto next=mon_pic::rgba(picture,battle::palette(ctx,current),list_mode&&selection.registration==dex_state::Registration::Seen);
    if(list_mode&&selection.registration==dex_state::Registration::Absent)next={};
    if(list_mode&&selection.number)for(int offset=-2;offset<=2;offset++) {
        int n=selection.number+offset;
        if(dex_state::flag(ctx,dex_state::Seen,n))cache.get(ctx->rom,ctx->rom_size,mon_pic::species(ctx->rom,ctx->rom_size,n),true);
    }
    if(next!=image) {image=next;glTexSubImage2D(GL_TEXTURE_2D,0,192,0,56,56,GL_RGBA,GL_UNSIGNED_BYTE,image.data());++uploads;}
    if(!lcd_valid||std::memcmp(lcd.data(),original,sizeof(lcd))) {
        std::copy_n(original,lcd.size(),lcd.begin());lcd_valid=true;
        std::array<uint8_t,160*144*4> rgba{};
        for(size_t i=0;i<lcd.size();i++){rgba[i*4]=lcd[i]>>16;rgba[i*4+1]=lcd[i]>>8;rgba[i*4+2]=lcd[i];rgba[i*4+3]=255;}
        glTexSubImage2D(GL_TEXTURE_2D,0,0,64,160,144,GL_RGBA,GL_UNSIGNED_BYTE,rgba.data());++uploads;
    }
    std::vector<Vertex> v;
    ...
    panel(v,.63f,2.8f,5.6f,5.65f,list_mode?Color{.50f,.61f,.53f}:Color{.095f,.17f,.15f},Solid,.38f);
    ...
    if(!list_mode) {
        ...
        for(int i=0;i<4;i++)panel(v,4.15f+i*.32f,3.53f,.14f,.48f,{.23f,.035f,.045f});
    }
    firstperson::Camera camera;float distance=std::max(6.7f,11.2f/(float(w)/h));
    camera.x=-distance*.105f;camera.y=.25f;camera.z=distance;camera.yaw=.105f;
    auto matrix=camera.perspective(float(w)/h);
    glViewport(0,0,w,h);glDisable(GL_SCISSOR_TEST);glDisable(GL_CULL_FACE);
    glClearColor(.035f,.065f,.075f,1);glDepthMask(GL_TRUE);glClearDepthf(1);
    ...
    glUniformMatrix4fv(matrix_loc,1,GL_FALSE,matrix.data());glUniform2f(fade_loc,1,0);
    glUniform1f(fog_loc,0);glUniform1f(sky_loc,0);glUniform1f(xray_loc,0);
