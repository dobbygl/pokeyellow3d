...
struct Vertex { Vec p; float u, v; Color c; };
...
struct Mesh { GLuint buffer=0; size_t count=0; std::vector<uint8_t> blocks,pending; };
...
UV tile_uv(const pallet::Scene& scene,int tile,int palette=0) {
    int slot=scene.interior?0:scene.tileset==3?1:scene.tileset==14?2:scene.tileset==23?3:0;
    return {float(slot*128+(tile%16)*8),float(palette*128+(tile/16)*8),8,8};
}
int floor_palette(const pallet::Scene& scene,int tile) {
    if(scene.interior)return interior::cave(scene)&&tile==0x14?2:0;
    if(tile==0x14)return 2;
    if(scene.tileset==0 && (tile==0x32 || tile==0x33 || tile==0x54))return 2;
    if(scene.tileset==14) {
        if(tile==0x10||tile==0x11)return 2;
        return 1;
    }
    if(scene.tileset==23 && (tile==0x32 || tile==0x33 || tile==0x1f))return 2;
    return 0;
}
std::vector<uint8_t> pixels(AW * AH * 4);
std::array<uint8_t,AW*32*4> uploaded_sprites{};
bool sprites_uploaded=false;
const Color ground[4] = {{.91f,.90f,.76f},{.71f,.77f,.52f},{.47f,.59f,.36f},{.23f,.34f,.25f}};
const Color water[4] = {{.64f,.87f,.86f},{.35f,.67f,.71f},{.19f,.47f,.57f},{.13f,.34f,.43f}};
const Color facade[4] = {{.98f,.92f,.75f},{.73f,.79f,.64f},{.37f,.54f,.49f},{.18f,.29f,.30f}};

void pixel(int x, int y, Color c) {
    size_t i = (y * AW + x) * 4;
    pixels[i] = uint8_t(c.r * 255); pixels[i+1] = uint8_t(c.g * 255);
    pixels[i+2] = uint8_t(c.b * 255); pixels[i+3] = uint8_t(c.a * 255);
}
Color shade(Color c, float f) { return {c.r*f,c.g*f,c.b*f,c.a}; }
...
void quad(std::vector<Vertex>& v, Vec a, Vec b, Vec c, Vec d, Color color = White, UV uv = Solid) {
    ...
    v.insert(v.end(), {{a,u0,v0,color},{b,u1,v0,color},{c,u1,v1,color},
                      {a,u0,v0,color},{c,u1,v1,color},{d,u0,v1,color}});
}

void detailed_quad(std::vector<Vertex>& v,Vec a,Vec b,Vec c,Vec d,Color color,UV detail) {
    if(detail.w==0) {quad(v,a,b,c,d,color);return;}
    ...
    auto length=[](Vec a,Vec b){return std::sqrt((a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y)+(a.z-b.z)*(a.z-b.z));};
    float u=length(a,b)/.75f,t=length(a,d)/.75f;
    v.insert(v.end(),{{a,0,0,color},{b,u,0,color},{c,u,t,color},
                     {a,0,0,color},{c,u,t,color},{d,0,t,color}});
}

void box(std::vector<Vertex>& v, float x, float z, float w, float d, float bottom, float top, Color c,UV detail=Solid) {
    detailed_quad(v,{x,top,z},{x+w,top,z},{x+w,top,z+d},{x,top,z+d},shade(c,1.08f),detail);
    detailed_quad(v,{x,top,z+d},{x+w,top,z+d},{x+w,bottom,z+d},{x,bottom,z+d},c,detail);
    detailed_quad(v,{x+w,top,z},{x,top,z},{x,bottom,z},{x+w,bottom,z},shade(c,.75f),detail);
    detailed_quad(v,{x,top,z},{x,top,z+d},{x,bottom,z+d},{x,bottom,z},shade(c,.85f),detail);
    detailed_quad(v,{x+w,top,z+d},{x+w,top,z},{x+w,bottom,z},{x+w,bottom,z+d},shade(c,.67f),detail);
}
...
void make_house(const pallet::Scene& scene,const pallet::House& h,const std::vector<uint8_t>& blocks) {
    const float x=h.x+scene.origin_x+.05f,z=h.z+scene.origin_z+.1f,w=h.w-.1f,d=h.d-.15f;
    const float eave=h.eave, ridge=eave+h.ridge;
    shadow(scenery,x+w*.5f+.3f,z+d*.65f,w*.62f,d*.65f);
    box(scenery,x,z,w,d,0,eave,{.88f,.83f,.66f},tile_uv(scene,scene.tileset==23?0x30:0x1a,1));
    // Original facade tiles remain individually addressable in the tileset atlas.
    int facade_rows=int((h.z+h.d)*2)-h.facade_row;
    for(int row=0;row<facade_rows;row++)for(int col=0;col<int(h.w)*2;col++) {
        int tile=pallet::map_tile(pallet::catalog_rom,scene,int(h.x)*2+col,h.facade_row+row,&blocks);
        float left=x+col*w/(h.w*2),right=x+(col+1)*w/(h.w*2);
        float top=eave-row*(eave-.03f)/facade_rows,bottom=eave-(row+1)*(eave-.03f)/facade_rows;
        quad(scenery,{left,top,z+d+.006f},{right,top,z+d+.006f},
            {right,bottom,z+d+.006f},{left,bottom,z+d+.006f},White,tile_uv(scene,tile,1));
    }
    Color roof = h.lab ? Color{.28f,.48f,.53f} : Color{.69f,.31f,.23f};
    float left=x-.17f,right=x+w+.17f,back=z-.14f,front=z+d+.17f,mid=z+d*.5f;
    quad(scenery,{left,ridge,mid},{right,ridge,mid},{right,eave,front},{left,eave,front},roof);
    quad(scenery,{right,ridge,mid},{left,ridge,mid},{left,eave,back},{right,eave,back},shade(roof,.7f));
    // Gable ends and a crisp ridge cap.
    quad(scenery,{left,eave,back},{left,ridge,mid},{left,eave,front},{left,eave,front},shade(roof,.83f));
    quad(scenery,{right,eave,front},{right,ridge,mid},{right,eave,back},{right,eave,back},shade(roof,.62f));
    ...
    for(int i=1;i<=5;i++) {
        ...
        quad(scenery,{left,yy,zz},{right,yy,zz},{right,yy-.025f,zz+.04f},{left,yy-.025f,zz+.04f},shade(roof,.82f));
    }
...
void create_atlas(const GBContext* ctx) {
    ...
    for(int id:current.interior?std::vector<int>{current.tileset}:std::vector<int>{0,3,14,23}) {
        const auto& tiles=pallet::catalog->tilesets.at(id);
        int slot=current.interior?0:id==3?1:id==14?2:id==23?3:0;
        Color room[4]={{.95f,.89f,.77f},{.74f,.70f,.56f},{.43f,.50f,.46f},{.16f,.23f,.25f}};
        if(id==6||id==2) {room[0]={.91f,.95f,.94f};room[1]={.65f,.80f,.80f};room[2]={.39f,.53f,.64f};}
        if(id==5||id==7||id==20||id==22) {room[0]={.92f,.94f,.84f};room[1]={.70f,.79f,.67f};room[2]={.43f,.58f,.53f};}
        if(id==17||id==11) {room[0]={.72f,.72f,.66f};room[1]={.48f,.52f,.49f};room[2]={.31f,.37f,.35f};}
        for(int tile=0;tile<96;tile++)for(int y=0;y<8;y++)for(int x=0;x<8;x++) {
            size_t at=tiles.graphics+tile*16+y*2;
            int value=((ctx->rom[at]>>(7-x))&1)|(((ctx->rom[at+1]>>(7-x))&1)<<1);
            for(int palette=0;palette<3;palette++)
                pixel(slot*128+(tile%16)*8+x,palette*128+(tile/16)*8+y,
                      (current.interior?(palette==2?water:room):(palette==0?ground:palette==1?facade:water))[value]);
        }
    }
    pixel(511,511,White);
...
void interior_map(const GBContext* ctx,const pallet::Scene& scene,const std::vector<uint8_t>& blocks) {
    for(int z=0;z<scene.height;z++)for(int x=0;x<scene.width;x++) {
        ...
        Color side{};side.r=side.g=side.b=0;side.a=1;
        for(int y=0;y<8;y++)for(int xx=0;xx<8;xx++) {
            size_t at=((int(uv.y)+y)*AW+int(uv.x)+xx)*4;
            side.r+=pixels[at]/(255.f*64);side.g+=pixels[at+1]/(255.f*64);side.b+=pixels[at+2]/(255.f*64);
        }
        ...
        for(int row=0;row<2;row++)for(int col=0;col<2;col++) {
            ...
            quad(scenery,{left,h,back},{left+.5f,h,back},{left+.5f,h,back+.5f},{left,h,back+.5f},White,tile_uv(scene,t));
            if(cell.kind==interior::Kind::Wall||cell.kind==interior::Kind::Counter) {
                float top=cell.height-row*cell.height*.5f,bottom=top-cell.height*.5f;
                quad(scenery,{left,top,z+1.002f},{left+.5f,top,z+1.002f},{left+.5f,bottom,z+1.002f},{left,bottom,z+1.002f},White,tile_uv(scene,t));
            }
        }
    }
}

void create_map(const GBContext* ctx,const pallet::Scene& scene,const std::vector<uint8_t>& blocks) {
    scenery.clear();
    for(int tz=0;tz<scene.height*2;tz++)for(int tx=0;tx<scene.width*2;tx++) {
        int tile=pallet::map_tile(ctx->rom,scene,tx,tz,&blocks);
        if(!scene.interior&&pallet::cleared(ctx->rom,scene,tx/2,tz/2,&blocks))tile=scene.tileset==3?0x30:scene.tileset==23?0x23:0x2c;
        float x=scene.origin_x+tx*.5f,z=scene.origin_z+tz*.5f;
        quad(scenery,{x,0,z},{x+.5f,0,z},{x+.5f,0,z+.5f},{x,0,z+.5f},White,tile_uv(scene,tile,floor_palette(scene,tile)));
    }
    if(scene.interior) {
        interior_map(ctx,scene,blocks);
        box(scenery,0,0,float(scene.width),float(scene.height),-.35f,-.015f,{.32f,.36f,.34f});
        return;
    }
    for(auto h:pallet::houses(scene))make_house(scene,h,blocks);
    for(int iz=0;iz<scene.height;iz++)for(int ix=0;ix<scene.width;ix++) {
        ...
        if(type==pallet::Terrain::TallTree) {
            ...
            box(scenery,x+.3f,z+.3f,1.4f,1.4f,1.8f,2.35f,{.30f,.49f,.30f},foliage);
            ...
            for(int row=0;row<2;row++)for(int col=0;col<2;col++) {
                int tile=pallet::map_tile(ctx->rom,scene,ix*2+col,iz*2+row,&blocks);
                float left=x+col*.5f,top=1.7f-row*.85f;
                quad(scenery,{left,top,z+1},{left+.5f,top,z+1},{left+.5f,top-.85f,z+1},{left,top-.85f,z+1},White,tile_uv(scene,tile,1));
            }
            ...
            box(scenery,x+.08f,z+.12f,.84f,.78f,.5f,1.10f,{.29f,.48f,.30f},foliage);
            box(scenery,x+.18f,z+.20f,.64f,.60f,1.10f,1.50f,{.38f,.56f,.34f},foliage);
            box(scenery,x+.29f,z+.30f,.42f,.4f,1.50f,1.68f,{.47f,.63f,.39f},foliage);
            ...
            if(bottom==0x27)box(scenery,x,z,.24f,1,0,.3f,{.57f,.47f,.30f});
            else if(bottom==0x0d||bottom==0x1d)box(scenery,x+.76f,z,.24f,1,0,.3f,{.57f,.47f,.30f});
            else {
            ...
            for(int i=0;i<4;i++) {
                ...
                quad(scenery,{px-.08f,.02f,pz},{px,.26f,pz},{px+.09f,.02f,pz},{px+.09f,.02f,pz},{.32f,.51f,.26f});
                quad(scenery,{px,.02f,pz-.08f},{px,.21f,pz},{px,.02f,pz+.09f},{px,.02f,pz+.09f},{.42f,.60f,.30f});
            }
        } else if(type==pallet::Terrain::Fence) {
            for(float off:{.16f,.66f})box(scenery,x+off,z+.55f,.12f,.14f,0,.65f,{.90f,.87f,.70f});
            ...
            for(int row=0;row<2;row++)for(int col=0;col<2;col++) {
                ...
                quad(scenery,{left,top,z+.566f},{left+.4f,top,z+.566f},
                    {left+.4f,top-.265f,z+.566f},{left,top-.265f,z+.566f},White,tile_uv(scene,tile,1));
            }
        }
    }
    box(scenery,scene.origin_x,scene.origin_z,float(scene.width),float(scene.height),-.8f,-.015f,{.42f,.48f,.34f});
}
...
void update_meshes(const GBContext* ctx) {
    ...
    for(auto it=meshes.begin();it!=meshes.end();) {
        if(std::find(visible_maps.begin(),visible_maps.end(),it->first)==visible_maps.end()) {
            glDeleteBuffers(1,&it->second.buffer);it=meshes.erase(it);
        } else ++it;
    }
    for(int id:visible_maps) {
        ...
        if(id==current.id && preview_map<0) {
            ...
            for(int z=0;z<bh;z++)for(int x=0;x<bw;x++)
                blocks[z*bw+x]=pallet::read(ctx,uint16_t(0xc6e8+(z+3)*(bw+6)+x+3));
        }
        auto& mesh=meshes[id];
        ...
        if(mesh.buffer && mesh.blocks==blocks)continue;
        auto start=std::chrono::steady_clock::now();
        create_map(ctx,scene,blocks);
        if(!mesh.buffer)glGenBuffers(1,&mesh.buffer);
        glBindBuffer(GL_ARRAY_BUFFER,mesh.buffer);
        glBufferData(GL_ARRAY_BUFFER,scenery.size()*sizeof(Vertex),scenery.data(),GL_STATIC_DRAW);
        mesh.count=scenery.size();mesh.blocks=std::move(blocks);++mesh_builds;
...
GLuint shader(GLenum type,const char* source) {
    ...
    if(!ok) { char log[2048];glGetShaderInfoLog(id,sizeof(log),nullptr,log);std::fprintf(stderr,"[3D] shader: %s\n",log);glDeleteShader(id);return 0; }
    return id;
}
...
bool initialize(const GBContext* ctx) {
    ...
    if(!a||!b) { if(a)glDeleteShader(a);if(b)glDeleteShader(b);return false; }
    program=glCreateProgram();glAttachShader(program,a);glAttachShader(program,b);
    glBindAttribLocation(program,0,"position");glBindAttribLocation(program,1,"texcoord");glBindAttribLocation(program,2,"color");
    glLinkProgram(program);glDeleteShader(a);glDeleteShader(b);
    GLint ok;glGetProgramiv(program,GL_LINK_STATUS,&ok);
    if(!ok) {char log[2048];glGetProgramInfoLog(program,sizeof(log),nullptr,log);std::fprintf(stderr,"[3D] shader link: %s\n",log);return false;}
...
void sprite_image(const GBContext* ctx,const pallet::Actor& actor) {
    ...
    for(int y=0;y<32;y++) for(int x=0;x<32;x++) pixel(ox+x,oy+y,{0,0,0,0});
    ...
    if(count>9||at+count*4>=ctx->rom_size)return;
    int base=image>>4;base=base==11?124:base*12;
    Color colors[4]={{0,0,0,0},{.97f,.91f,.73f},{.75f,.31f,.25f},{.16f,.20f,.22f}};
    if(actor.slot==15) {colors[1]={1,.88f,.27f};colors[2]={.77f,.47f,.15f};}
    else if(actor.slot!=0) colors[2]={.31f,.51f,.57f};
    for(int i=0;i<count;i++,at+=4) {
        ...
        if(tile>=128)tile=(tile+ctx->hram[0x7c])&255;
        for(int y=0;y<8;y++) for(int x=0;x<8;x++) {
            int tx=(flags&0x20)?7-x:x,ty=(flags&0x40)?7-y:y;
            int value=((ctx->vram[tile*16+ty*2]>>(7-tx))&1)|(((ctx->vram[tile*16+ty*2+1]>>(7-tx))&1)<<1);
            int px=8+dx+x,py=8+dy+y;
            if(px>=0&&px<32&&py>=0&&py<32)pixel(ox+px,oy+py,colors[value]);
        }
    }
}

void draw_world_frame(int w,int h,fade::Tone tone={},bool hide_hero=false,bool allow_xray=true,float approach=0,const firstperson::Matrix* override_matrix=nullptr);
...
void world(GBContext* ctx,int w,int h,fade::Tone tone={}) {
    ...
    if(atlas_tileset!=needed_atlas) {
        create_atlas(ctx);glBindTexture(GL_TEXTURE_2D,atlas);
    ...
    for(int slot=0;preview_map<0 && slot<16;slot++) {
        if(fp && slot==0)continue;
        pallet::Actor a;
        if(!pallet::actor(ctx,slot,a))continue;
        ...
        auto corner=[&](float side,float up) -> Vec {
            return {a.x+cs*side-sn*.78f*up,.6257795f*up,a.z-sn*side-cs*.78f*up};
        };
        ...
        if(fp) {
            ...
            float rx=len>.001f?-dz/len:std::cos(eye.yaw),rz=len>.001f?dx/len:std::sin(eye.yaw);
            int bottom=0;
            for(int y=0;y<32;y++)for(int x=0;x<32;x++)
                if(pixels[((384+y)*AW+slot*32+x)*4+3])bottom=y+1;
            float top=bottom*(1.9f/32),low=top-1.9f;
            tl={a.x-rx*half,top,a.z-rz*half};tr={a.x+rx*half,top,a.z+rz*half};
            bl={a.x-rx*half,low,a.z-rz*half};br={a.x+rx*half,low,a.z+rz*half};
        }
    ...
    if(!sprites_uploaded || std::memcmp(uploaded_sprites.data(),sprites,uploaded_sprites.size())) {
        glTexSubImage2D(GL_TEXTURE_2D,0,0,384,AW,32,GL_RGBA,GL_UNSIGNED_BYTE,sprites);
        std::memcpy(uploaded_sprites.data(),sprites,uploaded_sprites.size());sprites_uploaded=true;
    }
    const auto p=preview_map>=0?std::array<float,2>{current.origin_x+current.width*.5f,current.origin_z+current.height*.5f}:pallet::world_player(ctx);
    auto target=pallet::camera_target(p[0],p[1],zoom);
    if(current.interior) {
        float follow=std::clamp(room_zoom-1.f,0.f,1.f);
        target={current.width*.5f+(p[0]-current.width*.5f)*follow,current.height*.5f+(p[1]-current.height*.5f)*follow};
    }
    ...
    if(preview_map>=0||current.interior)unit=std::min(w/(current.width*std::abs(cs)+current.height*std::abs(sn)+5),
...
void draw_world_frame(int w,int h,fade::Tone tone,bool hide_hero,bool allow_xray,float approach,const firstperson::Matrix* override_matrix) {
    ...
    if(frame.fp) {
        ...
        glDisable(GL_DEPTH_TEST);glUniform1f(sky_loc,1);glDrawArrays(GL_TRIANGLES,0,6);
        glEnable(GL_DEPTH_TEST);glUniform1f(sky_loc,0);
    }
    for(int id:visible_maps) {
        const auto& mesh=meshes.at(id);bind_vertices(mesh.buffer);
...
void hud(const GBContext* ctx) {
    ...
    dl->AddRectFilledMultiColor({0,0},{io.DisplaySize.x,113*s},
        IM_COL32(13,25,28,235),IM_COL32(13,25,28,235),IM_COL32(13,25,28,0),IM_COL32(13,25,28,0));
    dl->AddRectFilledMultiColor({0,io.DisplaySize.y-65*s},io.DisplaySize,
        IM_COL32(13,25,28,0),IM_COL32(13,25,28,0),IM_COL32(13,25,28,245),IM_COL32(13,25,28,245));
    dl->AddText(nullptr,13*s,{25*s,22*s},dim,"KANTO  /  01");
    dl->AddText(nullptr,26*s,{25*s,43*s},bright,presented_scene(ctx)->title.c_str());
    dl->AddText(nullptr,13*s,{25*s,78*s},dim,first_person?"Amarillo  /  Primera persona":"Amarillo  /  Prototipo 3D");
    dl->AddText(nullptr,13*s,{25*s,io.DisplaySize.y-42*s},bright,first_person?"F2  2D / 3D     F3  Vista ortografica":"F2  2D / 3D     F3  Primera persona     Q / E  Girar     Rueda  Zoom");
    dl->AddText(nullptr,12*s,{25*s,io.DisplaySize.y-23*s},dim,first_person?"W  Avanzar   A / D  Girar   S  Media vuelta   Z  Hablar   Enter  Menu":"Flechas / WASD  Mover     Z  Hablar     Enter  Menu     Esc  Ajustes");
}
} // namespace
...
void pallet3d_draw(GBContext* ctx,int width,int height,bool menu_open) {
    ...
    if(!ctx||!ctx->wram||world_frame.map!=pallet::read(ctx,pallet::Map)||
       (pc_motion.initialized&&uint32_t(ctx->cycles)<pc_motion.last&&pc_motion.last-uint32_t(ctx->cycles)<0x80000000u))pc3d::reset();
    pc_sample=pc_state::sample(ctx);
    bool pc_wanted=enabled&&!failed&&preview_map<0&&world_frame.map>=0&&pc_sample.terminal.map==world_frame.map;
    if(pc_wanted)pc_terminal=pc_sample.terminal;
    ...
    if(std::getenv("PALLET3D_TRACE") && ctx && ctx->wram) {
        ...
        if(last!=int(state)||x!=lastx||y!=lasty) {
            std::fprintf(stderr,"[3D] state=%d map=%u xy=%d,%d font=%u sprites=%u bgp=%02x\n",int(state),pallet::read(ctx,pallet::Map),x,y,pallet::read(ctx,pallet::Font),pallet::read(ctx,pallet::UpdateSprites),ctx->io[0x47]);
    ...
    if(state==pallet::View::Pokedex&&preview_map<0) {
        ...
        if(!menu_overlay) {
            glViewport(0,0,width,height);glClearColor(.035f,.065f,.075f,1);glClear(GL_COLOR_BUFFER_BIT);
...
bool pallet3d_event(const SDL_Event* event,bool menu_open) {
    ...
    if(event->type==SDL_KEYDOWN && !event->key.repeat && event->key.keysym.scancode==SDL_SCANCODE_F2) {
    ...
    if(relative_allowed && (event->type==SDL_KEYDOWN || event->type==SDL_KEYUP)) {
        auto key=event->key.keysym.scancode;bool down=event->type==SDL_KEYDOWN;
        if(key==SDL_SCANCODE_W) {controls.forward=down;return true;}
        int amount=key==SDL_SCANCODE_A?-1:key==SDL_SCANCODE_D?1:key==SDL_SCANCODE_S?2:0;
        if(amount) {
            if(down && !event->key.repeat)controls.request_turn(pallet::read(input_context,0xc109),amount,input_context->cycles);
            return true;
        }
    }
    ...
    if(event->type==SDL_MOUSEWHEEL) {
        zoom=std::clamp(zoom+event->wheel.y*.12f,.7f,2.8f);return true;
    }
    if(event->type==SDL_KEYDOWN) {
        switch(event->key.keysym.scancode) {
        case SDL_SCANCODE_Q:yaw=std::max(-.75f,yaw-.08f);return true;
        case SDL_SCANCODE_E:yaw=std::min(.75f,yaw+.08f);return true;
        case SDL_SCANCODE_R:yaw=-.32f;zoom=1.f;return true;
        default:break;
        }
    }
    return false;
...
void pallet3d_capture(int width,int height) {
    ...
    if(!path||captured||!active||active_frames<30)return;
    std::vector<uint8_t> rgba(size_t(width)*height*4);
    glReadPixels(0,0,width,height,GL_RGBA,GL_UNSIGNED_BYTE,rgba.data());
    FILE* f=std::fopen(path,"wb");if(!f)return;
    std::fprintf(f,"P6\n%d %d\n255\n",width,height);
    for(int y=height-1;y>=0;y--)for(int x=0;x<width;x++)std::fwrite(&rgba[(y*width+x)*4],1,3,f);
...
Pallet3DStats pallet3d_stats() {
    size_t count=0;for(const auto& entry:meshes)count+=entry.second.count;
    return {meshes.size(),mesh_builds,count,count*sizeof(Vertex)};
}
void pallet3d_preview(int map_id) {preview_map=map_id;camera_ready=false;}

bool pallet3d_firstperson() {return first_person;}
...
void pallet3d_poll_controls(GBContext* ctx,bool menu_open) {
    ...
    if(std::getenv("PALLET3D_INPUT_TRACE") && ctx && previous_mask!=input_mask)
        std::fprintf(stderr,"[FPINPUT] cycle=%llu mask=%02x facing=%d idle=%d walk=%d moving=%d last=%d ready=%d\n",
            (unsigned long long)ctx->cycles,input_mask,facing,idle,pallet::read(ctx,pallet::Walk),pallet::read(ctx,0xd527),pallet::read(ctx,0xd528),pallet::read(ctx,0xcc4b));
...
PalletMenuInfo pallet3d_menu() {return {menu_overlay&&active&&!dex3d::presented&&!pc3d::presented,menu_full,menu_blurred,int(menu_regions.count)};}
...
PalletDexInfo pallet3d_dex() {
    return {active&&dex3d::presented,dex3d::presented&&dex3d::picture.valid,dex3d::species,
        battle::fingerprint(dex3d::image),dex3d::uploads,dex3d::list_mode,
        int(dex3d::selection.registration),dex3d::selection.number,dex3d::selection.seen,dex3d::selection.caught,
        dex3d::cache.resident(),dex3d::cache.decodes,dex3d::selection.row};
}
