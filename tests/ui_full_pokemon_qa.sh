...
set -euo pipefail
if (( $# != 3 )); then
    echo "Usage: $0 ROM TWO_POKEMON_WORLD READY_BATTLE" >&2
    exit 2
fi
...
sha256sum roms/pokeyellow.gbc source.state battle.state > logs/inputs.sha256
...
for mode in pokemon pokemon-fp pokemon-six pokemon-six-fp pokemon-pc pokemon-pc-fp pokemon-battle pokemon-battle-fp; do
    mkdir -p "$mode/logs"
    requested=$mode
    ...
    if [[ $mode == pokemon-battle* ]]; then fixture=battle.state; fi
...
rows = []
for mode in ('pokemon', 'pokemon-fp', 'pokemon-six', 'pokemon-six-fp', 'pokemon-pc', 'pokemon-pc-fp', 'pokemon-battle', 'pokemon-battle-fp'):
    text = (root / mode / 'logs/run.log').read_text()
    if '-battle' in mode:
        assert 'PASS: original battle party/actions' in text and 'FAIL' not in text, mode
        for page in ('party', 'actions', 'stats', 'moves'):
            assert (root / mode / 'logs' / f'pokemon-battle-{page}.state').is_file()
        row = {'mode': mode, 'source': 'battle'}
        if sys.argv[2] == 'integrated':
            ...
            assert all(row[key] > 0 for key in ('frames', 'glyphs', 'bits', 'cursors', 'party', 'actions', 'stats', 'moves')), row
        rows.append(row)
        continue
    if '-pc' in mode:
...
printf 'PASS: eight party/summary journeys, six-member/status/HP/level100, PC and battle sources in both cameras; evidence in %s\n' "$qa_dir" | tee logs/result.txt
