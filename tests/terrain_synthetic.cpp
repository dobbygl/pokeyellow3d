...
int main() {
    try {
        ...
        auto terrain=[&](synthetic::Cell c){return pallet::terrain(image.data(),*home,c.x,c.z);};
        ...
        check(terrain({plan.grass.x+1,plan.grass.z+1})==T::Grass,"grass covers the whole block");
        check(pallet::terrain(image.data(),*east,plan.east_grass.x,plan.east_grass.z)==T::Grass,
              "the eastern neighbour carries its own grass patch");

        auto cleared=[&](synthetic::Cell c){return pallet::cleared(image.data(),*home,c.x,c.z);};
        ...
        check(!cleared(plan.ground)&&!cleared(plan.grass),"walkable ground keeps its floor quad");
        check(!cleared(plan.water)&&!cleared(plan.ledge),"water and ledges keep their floor quad");
        ...
        check(house&&house->interior&&house->tileset==plan.interior_tileset,"interior scene available");
        using K=interior::Kind;
        auto cell=[&](synthetic::Cell c){return interior::classify(image.data(),*house,c.x,c.z);};
        auto floor=cell(plan.interior_floor);
        check(floor.kind==K::Floor&&floor.height==0&&floor.curated,"walkable floor stays flat and recognised");
        auto wall=cell(plan.interior_wall);
        check(wall.kind==K::Wall&&wall.height==plan.interior_wall_height&&wall.curated,"perimeter band is a wall");
        auto furniture=cell(plan.interior_furniture);
        check(furniture.kind==K::Furniture&&furniture.height==plan.interior_furniture_height&&furniture.curated,
        ...
        check(cell({plan.interior_floor.x+1,plan.interior_floor.z+1}).kind==K::Floor,"floor covers the whole block");
        check(cell({0,plan.interior_furniture.z}).kind==K::Wall,"the western band is a wall too");

        std::fprintf(stderr,"PASS: nine outdoor terrains, cleared quads and four interior cells on the synthetic world\n");
