...
inline void observe(GBContext* ctx,int frame) {
    ...
    if(!started)return;
    ++battle_frame;
    if(battle_frame==1){frozen=pallet3d_world_frame();meshes=pallet3d_stats();}
    ...
    if(phase.hud&&!first_hud)first_hud=battle_frame;
    if(presentation.arena&&!first_arena)first_arena=battle_frame;
    if(std::getenv("C2_VERIFY")) {
        if(!pallet3d_active()||!pallet3d_covers_frame(ctx)||pallet3d_input_mask()!=255) {
            std::fprintf(stderr,"[BATTLE-TIMING] uncovered frame=%d phase=%d flag=%d\n",battle_frame,int(phase.phase),flag);std::exit(42);
        }
        if(presentation.active&&(pallet3d_world_frame().camera!=frozen.camera||pallet3d_stats().mesh_builds!=meshes.mesh_builds)) {
            std::fprintf(stderr,"[BATTLE-TIMING] retained world changed frame=%d\n",battle_frame);std::exit(42);
        }
    }
    if(std::getenv("UI_VIDEO")) {
        ...
        if(!video) {
            std::string command="ffmpeg -v error -y -f rawvideo -pixel_format rgba -video_size "+std::to_string(w)+"x"+std::to_string(h)+
                " -framerate 60 -i - -vf vflip -an -c:v libx264 -preset veryfast -crf 22 logs/composed.mp4";
            video=popen(command.c_str(),"w");if(!video)std::exit(42);
        }
        std::vector<uint8_t> pixels(size_t(w)*h*4);glReadPixels(0,0,w,h,GL_RGBA,GL_UNSIGNED_BYTE,pixels.data());
        if(std::fwrite(pixels.data(),1,pixels.size(),video)!=pixels.size())std::exit(42);
    }
    if(last_phase!=int(phase.phase)||battle_frame==first_hud||battle_frame%60==0) {
        if(battle_frame<=1000) {
            char path[96];std::snprintf(path,sizeof(path),"logs/composed-%04d.ppm",battle_frame);capture_surface(path);
        }
    ...
    std::fprintf(trace,"%d,%d,%u,%d,%u,%u,%u,%u,%u,%u,%d,%d,%d,%d,%d,%d,%d,%d,%u,%u,%d,%.6f,%d,",
        ...
        battle::rectangle(ctx,battle::Player,true),battle::ready(ctx),pallet3d_covers_frame(ctx),presentation.arena,ctx->sp,ctx->rom_bank,
...
inline int run(GBContext* ctx,bool trainer,bool fp=false) {
    ...
    battle_frame=first_hud=first_arena=0;last_phase=-1;started=false;qa_frame_observer=observe;
    ...
    if(video&&pclose(video)!=0)return 42;video=nullptr;
    std::fprintf(stderr,"[BATTLE-TIMING] first HUD=%d first arena=%d\n",first_hud,first_arena);
    if(std::getenv("C2_VERIFY")&&(!first_hud||first_arena!=first_hud))return 42;
