...
int main() {
    try {
        ...
        for (size_t at = 0x134; at <= 0x14c; at++)
            header_sum = uint8_t(header_sum - image[at] - 1);
        ...
        check(world.connected_count == plan.connected_count,
              "home plus two neighbours are connected");
        check(world.maps.size() == plan.map_count, "forest and dock are always instantiated");
        for (int id : {plan.home, plan.north, plan.east, plan.forest, plan.dock})
            check(world.find(id), "expected map missing");
        ...
        check(home.connections[0].direction == 'N' && home.connections[0].map == plan.north,
              "northern exit");
        check(home.connections[0].x_alignment == plan.home_north_x_alignment, "northern alignment");
        check(home.connections[1].direction == 'E' && home.connections[1].map == plan.east,
              "eastern exit");
        ...
        check(north.origin_x == plan.north_origin_x && north.origin_z == plan.north_origin_z,
              "northern origin");
        check(east.origin_x == plan.east_origin_x && east.origin_z == plan.east_origin_z,
              "eastern origin");
        check(north.origin_z == -north.height,
              "the northern map sits exactly one map height above");
        ...
        check(north.connections.size() == 2 && north.connections[1].map == plan.east,
              "cycle edge 12 -> 1");
        check(east.connections.size() == 1 && east.connections[0].map == plan.home,
              "return edge 1 -> 0");
        ...
        check(home.warps[0].x == plan.home_warp.x && home.warps[0].z == plan.home_warp.z,
              "home warp position");
        check(home.warps[0].destination == plan.interior &&
                  home.warps[0].entrance == plan.home_warp.entrance,
              "home warp target");
        check(home.warps[1].destination == plan.home_dynamic_warp.destination,
              "dynamic warp sentinel");
        check(kanto::dynamic_warp(home, home.warps[1]), "the sentinel warp is not a room");
        check(home.signs.size() == 1 && home.signs[0].x == plan.home_sign.x &&
                  home.signs[0].z == plan.home_sign.z,
              "home sign");
        check(home.signs[0].text == plan.home_sign.text, "home sign text id");
        check(home.actors.size() == 2, "home actor count");
        check(home.actors[0].x == plan.home_actor.x && home.actors[0].z == plan.home_actor.z,
              "first actor position");
        check(home.actors[1].x == plan.home_item_actor.x &&
                  home.actors[1].z == plan.home_item_actor.z,
        ...
        check(probe.origin_x == 0 && probe.origin_z == 0,
              "interiors are not placed in the overworld");
        check(probe.tileset == plan.interior_tileset, "interior tileset");
        check(probe.width == plan.interior_width && probe.height == plan.interior_height,
              "interior dimensions");
        check(probe.warps.size() == 1 && probe.warps[0].destination == plan.home,
              "the interior warps back home");
        ...
        check(depths.at(plan.home) == 0 && depths.at(plan.dock) == 0,
              "resident maps are at depth zero");
        ...
        check(world.tilesets.at(plan.outdoor_tileset).grass == plan.grass_bottom,
              "grass tile from the tileset header");
        check(world.tilesets.at(plan.outdoor_tileset).walkable[plan.ground_bottom],
              "ground is walkable");
        check(!world.tilesets.at(plan.outdoor_tileset).walkable[plan.tree_bottom],
              "trees are not walkable");
        check(world.tilesets.at(plan.interior_tileset).walkable[plan.floor_bottom],
              "interior floor is walkable");

        // Tile sampling: top row and second row of every probed cell.
        auto tile = [&](const kanto::Map &m, synthetic::Cell c, int row) {
            return world.tile(rom, m, c.x * 2, c.z * 2 + row);
        };
        check(tile(home, plan.ground, 0) == plan.ground_top &&
                  tile(home, plan.ground, 1) == plan.ground_bottom,
              "ground tiles");
        check(tile(home, plan.grass, 1) == plan.grass_bottom, "grass tile");
        check(tile(home, plan.tree, 0) == plan.tree_top &&
                  tile(home, plan.tree, 1) == plan.tree_bottom,
              "tree tiles");
        check(tile(home, plan.rock, 0) == plan.rock_top &&
                  tile(home, plan.rock, 1) == plan.rock_bottom,
              "rock tiles");
        check(tile(home, plan.ledge, 1) == plan.ledge_bottom, "ledge tile");
        check(tile(home, plan.water, 1) == plan.water_bottom, "water tile");
        check(tile(home, plan.fence, 0) == plan.fence_top &&
                  tile(home, plan.fence, 1) == plan.fence_bottom,
              "fence tiles");
        check(tile(home, plan.signpost, 0) == plan.sign_top &&
                  tile(home, plan.signpost, 1) == plan.sign_bottom,
              "sign tiles");
        check(tile(home, plan.cut_tree, 0) == plan.cut_top &&
                  tile(home, plan.cut_tree, 1) == plan.cut_bottom,
              "cut tree tiles");
        check(tile(east, plan.east_grass, 1) == plan.grass_bottom,
              "grass on the eastern neighbour");
        ...
        check(tile(house, plan.interior_furniture, 0) == plan.furniture_top,
              "interior furniture tile");
        bool outside = false;
        try {
            world.tile(rom, home, home.width * 2, 0);
        ...
        auto reject = [&](std::vector<uint8_t> bad, const char *why) {
            bool rejected = false;
            try {
                kanto::World w(kanto::Rom(bad.data(), bad.size()));
            } catch (const std::exception &) {
                rejected = true;
            }
            check(rejected, why);
        ...
        auto bad = image;
        bad.resize(512);
        reject(bad, "a truncated image must be refused");
        bad = image;
        bad.push_back(0);
        reject(bad, "an oversized image must be refused");
        bad = image;
        bad[home.header + 4] = 0xff;
        reject(bad, "a block pointer outside the bank window must be refused");
        bad = image;
        bad[plan.outdoor_block_table] = 96;
        reject(bad, "a block holding a tile past the sheet must be refused");
        auto at = [](size_t offset) { return std::ptrdiff_t(offset); };
        bad = image;
        std::fill(bad.begin() + at(plan.ledge_table), bad.begin() + at(plan.ledge_table) + 256, 0);
        ...
        bad = image;
        bad[world.find(plan.north)->header + 10 + 11 + 7]++;
        reject(bad, "a contradictory connection cycle must be refused");

        std::fprintf(stderr,
                     "PASS: %zu connected maps, cycle origins, warps to depth %d, tiles and six "
                     "malformed images\n",
