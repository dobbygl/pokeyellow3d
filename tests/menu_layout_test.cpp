...
int main(int argc, char **argv) {
    ...
    for (int y = 0; y < 14; y++)
        for (int x = 10; x < 20; x++) {
            uint8_t tile = border({10, 0, 10, 14}, x, y);
            map[y * 20 + x] = tile ? tile : 0x7f;
        }
    auto start = classify(map.data());
    require(start.kind == Kind::Partial && start.count == 1, "Start window");
    map[19] = 0x7f;
    require(classify(map.data()).kind == Kind::Full, "missing corner is not a recognized layout");
    map[19] = 0x7b;
    map[0] = 0x80;
    require(classify(map.data()).kind == Kind::Full,
            "text outside recognized rectangles forces full screen");
    ...
    for (int i = 1; i < argc; i += 2) {
        ...
        require(f, "private tilemap fixture");
        require(std::fread(map.data(), 1, map.size(), f) == map.size() && std::fgetc(f) == EOF,
                "fixture has exactly 360 tiles");
        std::fclose(f);
