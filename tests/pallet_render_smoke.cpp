...
static void capture_surface(const char* path) {
    ...
    int w=viewport[2],h=viewport[3];std::vector<uint8_t> rgba(size_t(w)*h*4);
    glReadPixels(0,0,w,h,GL_RGBA,GL_UNSIGNED_BYTE,rgba.data());
    FILE* f=std::fopen(path,"wb");if(!f)std::exit(19);
    std::fprintf(f,"P6\n%d %d\n255\n",w,h);
    for(int y=h-1;y>=0;y--)for(int x=0;x<w;x++)std::fwrite(&rgba[(y*w+x)*4],1,3,f);
    std::fclose(f);
}
...
int main(int argc,char** argv) {
    ...
    size_t count=std::fread(rom.data(),1,rom.size(),f);std::fclose(f);
    if(count!=1048576)return 3;
    ...
    for(auto entry:POKEYELLOW_ASSETS_MANIFEST)
        std::memcpy(pokeyellow__rom_data+entry.rom_offset,rom.data()+entry.rom_offset,entry.size);
    ...
    if(argc>3 && (std::strcmp(argv[3],"firstperson")==0 || std::strcmp(argv[3],"fp-camera")==0)) {
        ...
        if(!pallet3d_event(&event,false)||!pallet3d_firstperson())return 27;
    }
    ...
    if(argc>3&&std::strcmp(argv[3],"battle-capture")==0) {
        int result=battle_capture(ctx);gb_platform_shutdown();return result;
    }
    if(argc>3&&std::strcmp(argv[3],"battle-swap")==0) {
        int result=battle_swap(ctx);gb_platform_shutdown();return result;
    }
    if(argc>3&&std::strcmp(argv[3],"trainer-prepare")==0) {
        int result=trainer_prepare(ctx);gb_platform_shutdown();return result;
    }
    if(argc>3&&std::strcmp(argv[3],"battle-trainer")==0) {
        int result=battle_trainer(ctx);gb_platform_shutdown();return result;
    }
    if(argc>3&&std::strcmp(argv[3],"battle-effects")==0) {
        int result=battle_effects(ctx,argv[2]);gb_platform_shutdown();return result;
    }
    if(argc>3&&std::strcmp(argv[3],"battle3d")==0) {
        if(battle_capture(ctx))return 40;
        if(!gb_context_load_state_file(ctx,"logs/capture-complete.state"))return 5;
        if(battle_swap(ctx))return 40;
        if(!gb_context_load_state_file(ctx,"logs/swap-return.state"))return 5;
        if(trainer_prepare(ctx))return 40;
        if(!gb_context_load_state_file(ctx,"logs/route22-trainer.state"))return 5;
        if(battle_trainer(ctx))return 40;
        if(battle_effects(ctx,"logs/capture-start.state"))return 40;
        gb_platform_shutdown();std::puts("PASS: battle3d capture, swaps, trainer, fainting, four effects and exact original fallback");return 0;
    }
    if(argc>3&&std::strcmp(argv[3],"town")==0) {
        int result=town_journey(ctx);gb_platform_shutdown();return result;
    }
    if(argc>3&&std::strcmp(argv[3],"interior-transitions")==0) {
        int result=interior_transitions(ctx);gb_platform_shutdown();return result;
    }
    if(argc>3&&std::strcmp(argv[3],"battle-inspect")==0) {
        std::fprintf(stderr,"[BATTLE-INSPECT] LCDC=%02x SCX=%d SCY=%d BGP=%02x ready=%d animation=%d SP=%04x bank=%d\n",ctx->io[0x40],ctx->io[0x43],ctx->io[0x42],ctx->io[0x47],battle::ready(ctx),battle::animation_running(ctx),ctx->sp,ctx->rom_bank);
        for(int y=0;y<7;y++) {
            std::fprintf(stderr,"[RECT] y=%d wram=",y);
            for(int x=0;x<7;x++)std::fprintf(stderr,"%02x ",battle::tile(ctx,12+x,y));
            std::fprintf(stderr,"vram9800=");for(int x=0;x<7;x++)std::fprintf(stderr,"%02x ",ctx->vram[0x1800+y*32+12+x]);
            std::fprintf(stderr,"vram9c00=");for(int x=0;x<7;x++)std::fprintf(stderr,"%02x ",ctx->vram[0x1c00+y*32+12+x]);
            std::fprintf(stderr,"\n");
        }
        std::fprintf(stderr,"[STACK]");for(int a=ctx->sp;a<0xe000;a++)std::fprintf(stderr," %02x",pallet::read(ctx,a));std::fprintf(stderr,"\n");
        gb_platform_shutdown();return 0;
    }
    if(argc>3 && (std::strcmp(argv[3],"fp-benchmark")==0||std::strcmp(argv[3],"ortho-benchmark")==0)) {int result=firstperson_benchmark(ctx,std::strcmp(argv[3],"fp-benchmark")==0);gb_platform_shutdown();return result;}
    if(argc>3 && std::strcmp(argv[3],"fp-views")==0) {int result=firstperson_views(ctx);gb_platform_shutdown();return result;}
    ...
    if(argc>6 && std::strcmp(argv[3],"warp")==0) {
        ...
        for(int frame=0;frame<350;frame++) {
            ...
            while(!ctx->frame_done) {
                ...
                if(!gb_platform_poll_events(ctx)||++slices>1000)return 12;
            }
            std::vector<uint8_t> before(ctx->wram,ctx->wram+8192);
            gb_platform_render_frame(gb_get_framebuffer(ctx));
            if(glGetError()!=GL_NO_ERROR || std::memcmp(before.data(),ctx->wram,8192))return 21;
        }
        std::fprintf(stderr,"[WARP] map=%u xy=%u,%u view=%d\n",pallet::read(ctx,pallet::Map),pallet::read(ctx,pallet::X),pallet::read(ctx,pallet::Y),int(pallet::view(ctx)));
        if(pallet::read(ctx,pallet::Map)!=std::atoi(argv[4]))return 24;
        if(!gb_context_save_state_file(ctx,argv[6]))return 18;
        gb_platform_shutdown();return 0;
    }
    if(argc>3 && (std::strcmp(argv[3],"catalog")==0||std::strcmp(argv[3],"interior-catalog")==0)) {
        bool interiors=!std::strcmp(argv[3],"interior-catalog");
        if(interiors)pallet::catalog->discover_warps(kanto::Rom(ctx->rom,ctx->rom_size));
        ReadOnlyMemory before{ctx};
        std::fprintf(stderr,"[CATALOG] GPU=%s\n",glGetString(GL_RENDERER));
        for(const auto& entry:pallet::catalog->maps) {
            if(interiors&&!entry.second.interior)continue;
            pallet3d_preview(entry.first);
            for(int frame=0;frame<3;frame++) {
                gb_platform_render_frame(gb_get_framebuffer(ctx));
                if(glGetError()!=GL_NO_ERROR || !pallet3d_active() || !before.unchanged(ctx))return 21;
            }
            auto stats=pallet3d_stats();
            if(stats.resident_maps!=1 || !stats.vertices)return 22;
            if(const char* benchmark=std::getenv("CATALOG_BENCH_FRAMES")) {
                int count=std::max(1,std::atoi(benchmark));glFinish();
                auto start=std::chrono::steady_clock::now();
                for(int i=0;i<count;i++) {gb_platform_render_frame(gb_get_framebuffer(ctx));glFinish();}
                double ms=std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-start).count()/count;
                std::fprintf(stderr,"[PERF] map=%d frames=%d mean_ms=%.3f\n",entry.first,count,ms);
            }
            if(const char* dir=std::getenv("CATALOG_CAPTURE_DIR")) {
                std::string path=std::string(dir)+"/map-"+std::to_string(entry.first)+".ppm";
                capture_surface(path.c_str());
            }
            std::fprintf(stderr,"[CATALOG] map=%d vertices=%zu bytes=%zu\n",entry.first,stats.vertices,stats.bytes);
        }
        pallet3d_preview(-1);gb_platform_render_frame(gb_get_framebuffer(ctx));
        if(!before.unchanged(ctx))return 21;
        gb_platform_shutdown();std::puts(interiors?"PASS: all 179 interior meshes, bounded eviction, OpenGL and read-only previews":"PASS: all 38 meshes, bounded eviction, OpenGL and read-only previews");return 0;
    }
    if(argc>3 && (std::strcmp(argv[3],"journey")==0 || std::strcmp(argv[3],"viridian")==0 || std::strcmp(argv[3],"firstperson")==0)) {
        int result=world_journey(ctx,std::strcmp(argv[3],"viridian")==0);gb_platform_shutdown();return result;
    }
    if(argc>3 && std::strcmp(argv[3],"turn-probe")==0) {
        QaWalk run{ctx};
        for(int duration=1;duration<=12;duration++) {
            if(!gb_context_load_state_file(ctx,argv[2]))return 5;
            run.input(nullptr);run.wait(30);
            int x=run.read(pallet::X),y=run.read(pallet::Y),facing=run.read(0xc109);
            run.press("U",duration);
            std::fprintf(stderr,"[TURN] duration=%d facing=%d -> %d delta=%d,%d\n",duration,facing,run.read(0xc109),run.read(pallet::X)-x,run.read(pallet::Y)-y);
        }
        gb_platform_shutdown();return 0;
    }
    if(argc>3 && std::strcmp(argv[3],"inspect")==0) {
        std::fprintf(stderr,"[MODE] bike/surf=%u standing=%02x facing=%u front=%02x\n",pallet::read(ctx,0xd6ff),ctx->hram[0x7b],pallet::read(ctx,0xc109),pallet::read(ctx,0xcfc5));
        if(auto* scene=pallet::scene(pallet::read(ctx,pallet::Map))) {
            int n=0,bw=scene->width/2;
            for(int z=0;z<scene->height/2;z++)for(int x=0;x<bw;x++) {
                int live=pallet::read(ctx,0xc6e8+(z+3)*(bw+6)+x+3),base=scene->block_data[z*bw+x];
                if(live!=base && n++<15)std::fprintf(stderr,"[BLOCK] xy=%d,%d rom=%02x live=%02x\n",x,z,base,live);
            }
            std::fprintf(stderr,"[BLOCK] differing=%d ptr=%02x%02x\n",n,pallet::read(ctx,0xd36a),pallet::read(ctx,0xd369));
        }
        gb_platform_shutdown();return 0;
    }
    if(argc>4 && std::strcmp(argv[3],"reload")==0) {
        auto render=[&]() {
            for(int i=0;i<4;i++) {
                std::vector<uint8_t> before(ctx->wram,ctx->wram+8192);
                gb_platform_render_frame(gb_get_framebuffer(ctx));
                if(glGetError()!=GL_NO_ERROR || std::memcmp(before.data(),ctx->wram,8192))std::exit(21);
            }
            return pallet3d_stats();
        };
        auto a=render();
        if(!gb_context_load_state_file(ctx,argv[4]))return 5;
        auto b=render();
        if(a.vertices<=b.vertices || b.mesh_builds!=a.mesh_builds+1)return 25;
        if(!gb_context_load_state_file(ctx,argv[2]))return 5;
        auto restored=render();
        if(restored.vertices!=a.vertices || restored.mesh_builds!=b.mesh_builds+1)return 26;
        gb_platform_shutdown();
        std::puts("PASS: load Cut result removes geometry, reload original restores it; only current mesh rebuilt, RAM read-only");return 0;
    }
    if(argc>5 && std::strcmp(argv[3],"play")==0) {
        ...
        std::fprintf(stderr,"[PLAY] map=%u xy=%u,%u battle=%u party=%u hp=%u font=%u\n",pallet::read(ctx,pallet::Map),pallet::read(ctx,pallet::X),pallet::read(ctx,pallet::Y),pallet::read(ctx,pallet::Battle),pallet::read(ctx,0xd162),pallet::read(ctx,0xd16b)*256+pallet::read(ctx,0xd16c),pallet::read(ctx,pallet::Font));
        if(argc>6 && !gb_context_save_state_file(ctx,argv[6]))return 18;
        if(const char* path=std::getenv("SMOKE_CAPTURE"))capture_surface(path);
        gb_platform_shutdown();return 0;
    }
    if(pallet::view(ctx)!=pallet::View::Overworld && pallet::view(ctx)!=pallet::View::Battle && !(argc>3 && std::strcmp(argv[3],"capture2d")==0))return 6;
    if(argc>3 && std::strcmp(argv[3],"route")==0) {
        if(pallet::read(ctx,pallet::X)!=9||pallet::read(ctx,pallet::Y)!=8)return 11;
        ...
        for(int frame=0;frame<450;frame++) {
            ...
            if(frame==155 && pallet::read(ctx,pallet::X)!=9)return 13;
            if(frame==250 && (pallet::read(ctx,pallet::X)!=5||pallet::read(ctx,pallet::Y)!=8))return 14;
            if(pallet::read(ctx,pallet::Map)==0 && pallet::read(ctx,pallet::Walk)>0 &&
               pallet::read(ctx,pallet::Font)==0 && ctx->io[0x47]==0xe4 && view!=pallet::View::Overworld)return 15;
            ReadOnlyMemory memory{ctx};gb_platform_render_frame(gb_get_framebuffer(ctx));
            if(!memory.unchanged(ctx))return 7;
            saw_inside_3d|=pallet::read(ctx,pallet::Map)==37&&pallet3d_active();
            if(glGetError()!=GL_NO_ERROR)return 16;
        }
        gb_platform_shutdown();
        if(!saw_menu||!saw_inside||!saw_inside_3d)return 17;
        std::puts("PASS: Start menu, return to 3D, wall collision, continuous walking, house entrance -> 3D");
        return 0;
    }
    std::vector<uint8_t> before(ctx->wram,ctx->wram+8192);
    for(int i=0;i<35;i++) {
        if(argc>3&&std::strcmp(argv[3],"camera")==0&&i==2) {
            SDL_Event ev{};ev.type=SDL_KEYDOWN;ev.key.keysym.scancode=SDL_SCANCODE_E;
            for(int j=0;j<10;j++)pallet3d_event(&ev,false);
    ...
    if(const char* path=std::getenv("SMOKE_CAPTURE"))capture_surface(path);
    if(std::memcmp(before.data(),ctx->wram,before.size()))return 7;
    // Check keyboard interception and fallback on the real SDL event path.
    SDL_Event toggle{};toggle.type=SDL_KEYDOWN;toggle.key.keysym.scancode=SDL_SCANCODE_F2;
    if(!pallet3d_event(&toggle,false))return 8;
    gb_platform_render_frame(gb_get_framebuffer(ctx));
    if(!pallet3d_event(&toggle,false))return 9;
    gb_platform_render_frame(gb_get_framebuffer(ctx));
    gb_platform_shutdown();
