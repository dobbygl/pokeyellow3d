...
struct Blend {
    ...
    bool begin(bool desired,bool frozen,uint32_t now,bool history) {
        if(!initialized){initialized=true;target=desired;last=now;paused=frozen;return false;}
        float delta=paused?0.f:std::min(25.f,float(uint32_t(now-last)));
        last=now;paused=frozen;
        if(frozen)return false;
        if(desired!=target) {
            target=desired;elapsed=0;running=history;
            return running; // Freeze the last completed (possibly mixed) frame.
        }
        if(running){elapsed=std::min(Duration,elapsed+delta);running=elapsed<Duration;}
        return false;
    }
