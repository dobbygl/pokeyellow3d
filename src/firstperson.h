...
inline int turn(int facing, int quarter_turns) {
    ...
    constexpr int compass[] = {4, 12, 0, 8};
    int index = 0;
    while (compass[index] != (facing & 12))
        ++index;
...
struct Controls {
    ...
    void request_turn(int facing, int amount, uint64_t cycles) {
        if (pending || cycles < rest_until)
            return;
        target = turn(facing, amount);
        pending = true;
        pulsing = false;
        attempts = 0;
    }
    uint8_t update(bool allowed, int facing, bool idle, uint64_t cycles) {
        if (!allowed || cycles < last_cycles) {
            reset();
            last_cycles = cycles;
            return 0xff;
        }
        ...
        if (pending && facing == target) {
            pending = false;
            pulsing = false;
            rest_until = cycles + TurnCycles;
            return 0xff;
        }
        if (cycles < rest_until)
            return 0xff;
        if (pending) {
            if (pulsing) {
                if (cycles - pulse_started < TurnCycles)
                    return dpad(target);
                pulsing = false;
                rest_until = cycles + TurnCycles;
                if (attempts >= 3)
...
inline float angle_delta(float from, float to) {
    return std::remainder(to - from, 2 * Pi);
}
...
struct Camera {
    ...
    void update(float px, float pz, int facing, float dt, uint64_t cycles, float jump = 0) {
        ...
        if (!ready || cycles < last_cycles || std::abs(px - x) + std::abs(pz - z) > 3)
            yaw = target;
        else
            yaw += angle_delta(yaw, target) * (1 - std::exp(-std::clamp(dt, 0.f, .1f) / .15f));
        yaw = std::remainder(yaw, 2 * Pi);
        x = px;
        z = pz;
        y = 1.1f + jump;
        ready = true;
