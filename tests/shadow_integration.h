...
inline int captures(GBContext *ctx, bool fp) {
    ...
    for (double hour : {-1., 0., 6., 6.5, 12., 17.5, 18., 21.}) {
        ...
        for (int repeat = 0; repeat < 3; ++repeat) {
            run.require(presentation_qa::render(ctx) == image,
                        "frozen shadow image is deterministic");
        ...
        run.require(presentation_qa::render(ctx) == reference,
                    "toggle restores every reference byte for this hour");
    }
    ...
    run.require(pallet3d_artistic(true) && pallet3d_daylight({daynight::Mode::Fixed, 12}),
                "lit pause fixture");
    auto stable = presentation_qa::render(ctx);
    ...
    for (int i = 0; i < 3; ++i) {
        SDL_Delay(40);
        run.require(presentation_qa::render(ctx) == stable && pallet3d_shadows().passes == passes,
                    "focus loss preserves the image and cached depth");
    }
    event.window.event = SDL_WINDOWEVENT_FOCUS_GAINED;
    pallet3d_event(&event, false);
    run.require(presentation_qa::render(ctx) == stable, "focus return preserves shadow phase");
    ...
    special_transition_qa::key(ctx, SDL_SCANCODE_ESCAPE);
    run.require(presentation_qa::render(ctx) == stable, "closing Esc restores identical lit image");
    run.require(world_animation_qa::snapshot(run, "logs/shadow-after.state") == state,
