...
inline void shutdown() {
    ...
    texture=0;valid.fill(false);uploads=bytes=0;
}
inline bool upload(const uint32_t* framebuffer,Rect r) {
    if(!framebuffer||r.x<0||r.y<0||r.w<=0||r.h<=0||r.x+r.w>20||r.y+r.h>18)return false;
    if(!texture) {
    ...
    bool changed=false;
    for(int y=r.y*8;y<(r.y+r.h)*8;y++)for(int x=r.x*8;x<(r.x+r.w)*8;x++) {
        int i=y*160+x;changed|=!valid[i]||cached[i]!=framebuffer[i];
    }
    if(changed) {
        std::vector<uint8_t> rgba(size_t(r.w)*r.h*64*4);size_t p=0;
        for(int y=r.y*8;y<(r.y+r.h)*8;y++)for(int x=r.x*8;x<(r.x+r.w)*8;x++) {
            int i=y*160+x;uint32_t c=framebuffer[i];cached[i]=c;valid[i]=true;
            rgba[p++]=uint8_t(c>>16);rgba[p++]=uint8_t(c>>8);rgba[p++]=uint8_t(c);rgba[p++]=255;
        }
        glBindTexture(GL_TEXTURE_2D,texture);
        glTexSubImage2D(GL_TEXTURE_2D,0,r.x*8,r.y*8,r.w*8,r.h*8,GL_RGBA,GL_UNSIGNED_BYTE,rgba.data());
        ++uploads;bytes+=rgba.size();
    }
    glBindTexture(GL_TEXTURE_2D,0);return texture!=0;
}
inline void draw(const uint32_t* framebuffer,Rect r,float w,float h,float alpha=1,bool bottom_aligned=false) {
    if(!upload(framebuffer,r))return;
    float scale=std::max(1.f,std::floor(std::min(w/160,h/144)));
    float left=(w-160*scale)*.5f,top=(h-144*scale)*(bottom_aligned?1.f:.5f);
    ImGui::GetForegroundDrawList()->AddImage((ImTextureID)(intptr_t)texture,
        {left+r.x*8*scale,top+r.y*8*scale},{left+(r.x+r.w)*8*scale,top+(r.y+r.h)*8*scale},
        {r.x/20.f,r.y/18.f},{(r.x+r.w)/20.f,(r.y+r.h)/18.f},
...
inline void regions(const uint32_t* framebuffer,const menu_layout::Layout& layout,float w,float h) {
    ...
    for(size_t i=0;i<layout.count;i++) {
        auto r=layout.regions[i];float x=left+r.x*8*scale,y=top+r.y*8*scale;
        for(int pad=3;pad>=1;pad--)
            dl->AddRectFilled({x-pad,y-pad+2},{x+r.w*8*scale+pad,y+r.h*8*scale+pad+2},IM_COL32(0,0,0,35));
    }
    for(size_t i=0;i<layout.count;i++) {
        auto r=layout.regions[i];draw(framebuffer,{r.x,r.y,r.w,r.h},w,h,1,true);
    }
}
} // namespace lcd_overlay
