...
inline int world_journey(GBContext *ctx, bool viridian = false) {
    ...
    auto require = [](bool ok, const char *message) {
        if (!ok) {
            std::fprintf(stderr, "[JOURNEY] FAIL: %s\n", message);
            std::exit(30);
        }
    };
    require(byte(pallet::Map) == 0 && byte(pallet::X) == 9 && byte(pallet::Y) == 7 &&
                byte(0xd162) > 0,
    ...
    bool previous_3d = false;
    int fp_frames = 0, jump_frames = 0, battle_3d = 0;
    auto tick = [&]() {
        auto prev = pallet::world_player(ctx);
        int prev_map = byte(pallet::Map);
        gb_reset_frame(ctx);
        ctx->stopped = 0;
        unsigned slices = 0;
        ...
        if (prev_map != byte(pallet::Map)) {
            ...
            require(pallet::scene(prev_map) && pallet::scene(byte(pallet::Map)),
                    "Journey stays in supported maps");
            require(std::abs(prev[0] - now[0]) + std::abs(prev[1] - now[1]) < 1.5f,
                    "No coordinate jump at map seam");
            ++crossings;
        }
        ...
        if (state == pallet::View::Overworld)
            ++overworld_frames;
        ReadOnlyMemory before{ctx};
        gb_platform_render_frame(gb_get_framebuffer(ctx));
        ++frame;
        require(glGetError() == GL_NO_ERROR, "No OpenGL error");
        require(pallet3d_stats().resident_maps <= 5,
                "Mesh cache bounded to current plus four neighbors");
        require(before.unchanged(ctx), "Renderer leaves WRAM, VRAM and framebuffer unchanged");
        require(pallet3d_active() ==
                    (pallet3d_blend().active || state == pallet::View::Overworld ||
                     state == pallet::View::Battle || pallet3d_warp_overlay() ||
                     pallet3d_battle_transition().active || pallet3d_menu().active ||
                     (state == pallet::View::Dialogue && pallet::bottom_dialogue(ctx))),
                "Real renderer follows scene selection");
        if (state == pallet::View::Battle)
            ++battle_3d;
        if (pallet3d_firstperson() && state == pallet::View::Overworld) {
            auto camera = pallet3d_camera();
            auto position = pallet::world_player(ctx);
            ++fp_frames;
            require(std::abs(camera.x - position[0]) < .0001f &&
                        std::abs(camera.z - position[1]) < .0001f,
                    "FP camera follows interpolated engine position exactly");
            require(!camera.player_drawn, "FP never draws the player or its x-ray");
            require(camera.y >= 1.099f && camera.y <= 1.241f,
                    "Ledge bob stays within its visual height range");
            if (camera.y > 1.101f)
                ++jump_frames;
            if (!previous_3d)
                require(std::abs(firstperson::angle_delta(
                            camera.yaw, firstperson::facing_yaw(byte(0xc109)))) < .0001f,
                        "Return from 2D restores engine-facing camera immediately");
        }
        if (state != pallet::View::Overworld)
            require(pallet3d_input_mask() == 255, "Relative dpad is neutral outside overworld");
        previous_3d = pallet3d_active() && state != pallet::View::Battle &&
                      !pallet3d_battle_transition().active;
    };
        ...
        for (int i = 0; i < 5000; i++) {
            ...
            if (!byte(pallet::Battle) && pallet::view(ctx) == pallet::View::Overworld) {
                ...
                std::fprintf(stderr, "[JOURNEY] victory=%d frame=%d exp=%d map=%u xy=%u,%u\n",
                             victories, frame, experience(), byte(pallet::Map), byte(pallet::X),
                             byte(pallet::Y));
                return;
            }
        }
    ...
    auto move = [&](int map, int x, int y, const char *direction) {
        ...
        for (int i = 0; i < 1200; i++) {
            ...
            if (byte(pallet::Map) == map && byte(pallet::X) == x && byte(pallet::Y) == y &&
                !byte(pallet::Walk)) {
                input(nullptr);
        ...
        std::fprintf(stderr, "[JOURNEY] stuck map=%u xy=%u,%u target=%d,%d,%d\n", byte(pallet::Map),
                     byte(pallet::X), byte(pallet::Y), map, x, y);
    ...
    require(victories > 0 && battle_3d > 100,
            "Encounter and defeat a wild Pokemon with the 3D scene active");
    if (viridian) {
        move(12, 8, 28, "L");
        move(12, 8, 24, "U");
        move(12, 12, 24, "R");
    ...
    require(crossings == (viridian ? 4 : 2) && overworld_frames > 200,
            "Cross both ways and render the connected overworld");
    if (viridian)
        std::fprintf(
            stderr,
            "PASS: Viridian round trip, four seam crossings including horizontal displacement\n");
    std::fprintf(stderr,
                 "PASS: Pallet -> Route 1, ledge blocks north/jumps south, %d real wild victories, "
                 "3D combat -> overworld, Route 1 -> Pallet; frames=%d 3D=%d battle=%d "
                 "battle3d=%d, GL and memory checks every frame\n",
                 victories, frame, overworld_frames, battle_frames, battle_3d);
    if (pallet3d_firstperson()) {
        require(fp_frames > 200 && jump_frames > 0,
                "First-person journey includes a real ledge bob");
        std::fprintf(stderr, "[FP] frames=%d jump_frames=%d\n", fp_frames, jump_frames);
    }
    uint64_t digest = 1469598103934665603ull;
    for (int i = 0; i < 8192; i++)
        digest = (digest ^ ctx->wram[i]) * 1099511628211ull;
