...
class Rom {
  ...
  public:
    Rom(const uint8_t *bytes, size_t size) : bytes_(bytes), size_(size) {
        if (!bytes || size != RomSize)
            throw std::runtime_error("Expected 1 MiB UE Yellow ROM");
    }
    void range(size_t at, size_t count) const {
        if (at > size_ || count > size_ - at)
            throw std::runtime_error("ROM range exceeds image");
    }
    uint8_t byte(size_t at) const {
        range(at, 1);
        return bytes_[at];
    }
    uint16_t word(size_t at) const {
        range(at, 2);
        return bytes_[at] | (bytes_[at + 1] << 8);
    }
    size_t address(unsigned bank, unsigned pointer) const {
        if (bank >= size_ / 0x4000 || pointer >= 0x8000)
            throw std::runtime_error("Invalid ROM bank/pointer");
        return pointer < 0x4000 ? pointer : bank * 0x4000 + pointer - 0x4000;
    }
    std::vector<uint8_t> copy(size_t at, size_t count) const {
        range(at, count);
        return {bytes_ + at, bytes_ + at + count};
    }
};
struct Tileset {
...
struct Connection {
    char direction;
    int map, y_alignment, x_alignment;
};
struct Warp {
    int x, z, destination, entrance;
};
struct Sign {
    int x, z, text;
};
struct Object {
    int sprite, x, z, movement, range, text;
};
struct Map {
    int id = 0, bank = 0, tileset = 0, width = 0, height = 0, origin_x = 0, origin_z = 0,
        component = -1, border = 0;
...
inline std::string title(int id) {
    static const char *cities[] = {"PUEBLO PALETA",  "CIUDAD VERDE",   "CIUDAD PLATEADA",
                                   "CIUDAD CELESTE", "PUEBLO LAVANDA", "CIUDAD CARMIN",
                                   "CIUDAD AZULONA", "CIUDAD FUCSIA",  "ISLA CANELA",
                                   "MESETA ANIL",    "CIUDAD AZAFRAN"};
    if (id >= 0 && id < 11)
        return cities[id];
    if (id >= 12 && id <= 36)
        return "RUTA " + std::to_string(id - 11);
    if (id == 51)
        return "BOSQUE VERDE";
    if (id == 94)
        return "MUELLE DE CARMIN";
    if (id >= 0 && id < MapCount) {
        std::string name = MapNames[id];
...
inline Tileset read_tileset(const Rom &rom, int id) {
    if (id < 0 || id >= TilesetCount)
        throw std::runtime_error("Invalid tileset ID");
    ...
    for (int i = 0; i < 3; i++)
        t.counters[i] = rom.byte(p + 7 + i);
    t.grass = rom.byte(p + 10);
    t.animations = rom.byte(p + 11);
...
inline Map read_map(const Rom &rom, int id) {
    if (id < 0 || id >= MapCount || std::string(MapNames[id]).find("UNUSED_") == 0)
        throw std::runtime_error("Invalid or unused map ID " + std::to_string(id));
    Map m;
    m.id = id;
    m.title = title(id);
    m.bank = rom.byte(HeaderBanks + id);
    m.header = rom.address(m.bank, rom.word(HeaderPointers + id * 2));
    size_t p = m.header;
    m.tileset = rom.byte(p);
    m.height = rom.byte(p + 1) * 2;
    m.width = rom.byte(p + 2) * 2;
    ...
    if (flags & 0xf0)
        throw std::runtime_error("Invalid connection flags");
    p += 10;
    ...
    for (int i = 0; i < 4; i++)
        if (flags & (8 >> i)) {
            ...
            if (destination >= MapCount || destination == 11)
                throw std::runtime_error("Invalid connected map");
            m.connections.push_back(
                {directions[i], destination, int8_t(rom.byte(p + 7)), int8_t(rom.byte(p + 8))});
            p += 11;
        }
    m.objects = rom.address(m.bank, rom.word(p));
    p = m.objects;
    m.border = rom.byte(p++);
    int n = rom.byte(p++);
    if (n > 32)
        throw std::runtime_error("Excessive warp count");
    for (int i = 0; i < n; i++, p += 4) {
        Warp w{rom.byte(p + 1), rom.byte(p), rom.byte(p + 3), rom.byte(p + 2)};
        if (w.x >= m.width || w.z >= m.height)
            throw std::runtime_error("Warp outside map");
        m.warps.push_back(w);
    }
    n = rom.byte(p++);
    if (n > 32)
        throw std::runtime_error("Excessive sign count");
    for (int i = 0; i < n; i++, p += 3) {
        Sign s{rom.byte(p + 1), rom.byte(p), rom.byte(p + 2)};
        if (s.x >= m.width || s.z >= m.height)
            throw std::runtime_error("Sign outside map");
        m.signs.push_back(s);
    }
    n = rom.byte(p++);
    if (n > 16)
        throw std::runtime_error("Excessive actor count");
    for (int i = 0; i < n; i++) {
        Object o{rom.byte(p),     int(rom.byte(p + 2)) - 4, int(rom.byte(p + 1)) - 4,
                 rom.byte(p + 3), rom.byte(p + 4),          rom.byte(p + 5)};
        p += 6;
        rom.range(p, 0);
        if (o.text & 0x40) {
            rom.range(p, 2);
...
class World {
  public:
    ...
    explicit World(const Rom &rom) {
        maps.emplace(0, read_map(rom, 0));
        maps.at(0).component = 0;
        std::queue<int> queue;
        queue.push(0);
        while (!queue.empty()) {
            int id = queue.front();
            ...
            for (auto c : a.connections) {
                ...
                if (fresh)
                    maps.emplace(c.map, read_map(rom, c.map));
                ...
                if (!fresh && (x != b.origin_x || z != b.origin_z))
                    throw std::runtime_error("Contradictory connection cycle");
                b.origin_x = x;
                b.origin_z = z;
                b.component = 0;
                if (fresh)
        ...
        for (int id : {51, 94}) {
            auto m = read_map(rom, id);
            m.component = id;
            maps.emplace(id, std::move(m));
        }
        for (const auto &entry : maps) {
            const auto &m = entry.second;
            if (!tilesets.count(m.tileset))
                tilesets.emplace(m.tileset, read_tileset(rom, m.tileset));
            const auto &t = tilesets.at(m.tileset);
            for (auto block : m.block_data)
                for (int i = 0; i < 16; i++) {
                    int tile = rom.byte(t.blocks + block * 16 + i);
                    if (tile >= 96)
                        throw std::runtime_error("Unexpected graphics tile in map " +
                                                 std::to_string(m.id));
    ...
    const Map &ensure_map(const Rom &rom, int id) {
        ...
        if (!tilesets.count(m.tileset))
            tilesets.emplace(m.tileset, read_tileset(rom, m.tileset));
        const auto &t = tilesets.at(m.tileset);
        for (auto b : m.block_data)
            if (!t.valid_blocks[b])
                throw std::runtime_error("Invalid block in map " + std::to_string(id));
    ...
    std::map<int, int> discover_warps(const Rom &rom, int max_depth = 32) {
        ...
        while (!queue.empty()) {
            ...
            for (const auto &warp : m.warps) {
                int next = warp.destination;
                if (dynamic_warp(m, warp) || depths.count(next))
                    continue;
                if (depths.at(id) >= max_depth)
                    throw std::runtime_error("Warp graph exceeds depth limit");
                try {
                    ensure_map(rom, next);
                } catch (const std::exception &e) {
                    throw std::runtime_error("Warp " + std::to_string(id) + " -> " +
                                             std::to_string(next) + ": " + e.what());
                }
                depths[next] = depths.at(id) + 1;
                queue.push(next);
            }
        }
        return depths;
    }
    uint8_t tile(const Rom &rom, const Map &m, int tx, int tz,
                 const std::vector<uint8_t> *live = nullptr) const {
        if (tx < 0 || tz < 0 || tx >= m.width * 2 || tz >= m.height * 2)
            throw std::runtime_error("Tile outside map");
