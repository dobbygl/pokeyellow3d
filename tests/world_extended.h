...
struct QaWalk {
    ...
    void tick() {
        int previous = read(pallet::Map);
        auto old = pallet::world_player(ctx);
        gb_reset_frame(ctx);
        ctx->stopped = 0;
        unsigned slices = 0;
        ...
        if (previous != read(pallet::Map)) {
            ...
            const auto *a = pallet::scene(previous);
            const auto *b = pallet::scene(read(pallet::Map));
            bool respawn = expected_respawn == read(pallet::Map);
            if (respawn)
                expected_respawn = -1;
            if (a && b && a->component == b->component && !respawn)
                require(std::abs(old[0] - current[0]) + std::abs(old[1] - current[1]) < 1.5f,
                        "continuous connection origin");
            std::fprintf(stderr, "[EXTENDED] crossing %d -> %d xy=%d,%d delta=%.2f\n", previous,
                         read(pallet::Map), read(pallet::X), read(pallet::Y),
                         std::abs(old[0] - current[0]) + std::abs(old[1] - current[1]));
        }
        ...
        if (const char *path = std::getenv("UI_TRACE")) {
            ...
            if (!trace) {
                ...
                require(trace, "UI frame trace");
                std::fputs("frame,cycles,map,x,y,view,bgp,lcdc,font,sprites,battle,covered,active,"
                           "dialogue,warp,hero_image,sp\n",
                           trace);
            }
            std::fprintf(trace, "%d,%u,%d,%d,%d,%d,%u,%u,%d,%d,%d,%d,%d,%d,%d,%d,%u\n", frame,
                         ctx->cycles, read(pallet::Map), read(pallet::X), read(pallet::Y),
                         int(state), ctx->io[0x47], ctx->io[0x40], read(pallet::Font),
                         read(pallet::UpdateSprites), read(pallet::Battle),
                         pallet3d_covers_frame(ctx), pallet3d_active(), pallet3d_dialogue_overlay(),
                         pallet3d_warp_overlay(), read(pallet::Sprite1 + 2), ctx->sp);
        }
        ...
        if (pallet3d_firstperson() && state == pallet::View::Overworld && pallet3d_active() &&
            (!blend.active || blend.target_3d) && !previous_3d)
            require(std::abs(firstperson::angle_delta(
                        pallet3d_camera().yaw, firstperson::facing_yaw(read(0xc109)))) < .0001f,
                    "first frame returning to FP matches engine facing");
        ...
        previous_3d = pallet3d_active() && (!blend.active || blend.target_3d) &&
                      state != pallet::View::Battle && !pallet3d_battle_transition().active;
        if (qa_frame_observer)
            qa_frame_observer(ctx, frame);
    }
    void wait(int count) {
        for (int i = 0; i < count; i++)
            tick();
    }
    void press(const char *button, int count = 4) {
        input(button);
        wait(count);
        input(nullptr);
        wait(30);
    ...
    void move(int map, int x, int y, const char *direction) {
        ...
        for (int i = 0; i < 1000; i++) {
            tick();
            require(!read(pallet::Battle), "fixture path avoids encounters");
            if (read(pallet::Map) == map && read(pallet::X) == x && read(pallet::Y) == y &&
                !read(pallet::Walk)) {
                input(nullptr);
                return;
            }
...
inline int extended_prepare(GBContext *ctx, const char *scenario, const char *output) {
    QaWalk run{ctx};
    auto set = [&](int address, int value) { ctx->wram[address - 0xc000] = uint8_t(value); };
    if (!std::strcmp(scenario, "cut") || !std::strcmp(scenario, "horizontal")) {
        run.require(run.read(pallet::Map) == 1 && run.read(pallet::X) == 20 &&
                        run.read(pallet::Y) == 33,
                    "real Viridian fixture");
        run.move(1, 20, 30, "U");
        run.move(1, 19, 30, "L");
        run.move(1, 19, 21, "U");
        if (!std::strcmp(scenario, "horizontal")) {
            ...
            run.press("L");
            set(0xd163, 0x99);
            set(0xd16a, 0x99); // Bulbasaur, compatible with Cut.
            set(0xd172, 15);
            set(0xd355, run.read(0xd355) | 2); // Cut, Cascade Badge.
        }
    } else if (!std::strcmp(scenario, "surf")) {
        run.require(run.read(pallet::Map) == 0 && run.read(pallet::X) == 9 &&
                        run.read(pallet::Y) == 7,
                    "Pallet fixture");
        run.move(0, 9, 13, "D");
        run.move(0, 6, 13, "L");
        run.press("D");
        set(0xd172, 57);
        set(0xd355, run.read(0xd355) | 16); // Surf Pikachu, Soul Badge.
        ...
        run.require(run.read(pallet::Map) == 0 && (mode == 1 || mode == 2),
        ...
        if (mode == 1) {
            ...
            run.move(0, 6, 15, "D");
            run.require(run.read(0xd6ff) == 2, "surf remains active on water");
            run.move(0, 6, 13, "U");
            run.require(run.read(0xd6ff) == 0, "original engine dismounts at shore");
        }
