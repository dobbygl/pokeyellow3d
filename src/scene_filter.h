...
inline void shutdown() {
    ...
    program=texture=buffer=0;width=height=0;valid=false;
}
inline bool initialize() {
    ...
    glDeleteShader(a);glDeleteShader(b);GLint ok=0;glGetProgramiv(program,GL_LINK_STATUS,&ok);
...
inline bool capture(int w,int h) {
    ...
    if(width!=w||height!=h) {
        width=w;height=h;glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA,w,h,0,GL_RGBA,GL_UNSIGNED_BYTE,nullptr);
    }
